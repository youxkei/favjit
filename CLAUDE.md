...
- [crates/](crates/) — the cargo workspace: `engine`, a host per platform, the
...
- [docs/adr/](docs/adr/) — the architecture decisions, one per file, with the
  reasoning that led to each. Its README describes the process and says what counts
  as one; read it before writing one. Don't restate that reasoning at length in code
  comments — cite the ADR instead.
- [docs/platform/](docs/platform/) — what the OS input APIs actually do, one
  ...
  version it was observed on, never asserted from memory. **What favjit does about
  a platform is here as well**, beside the finding that settled it: "on this machine,
  do it this way" is not an architecture decision, so it does not go in an ADR.
