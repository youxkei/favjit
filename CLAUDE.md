...
- **Every impure operation goes behind the per-platform `host` facade** — OS input, transport, clock, and the supply of events ([ADR-0006](docs/adr/0006-host-boundary.md)). `engine` never touches `std::thread`, `std::time`, or the network. Hosts stay thin: whatever lives inside one is reachable only by real-hardware testing.
...
- **`engine` holds no concurrency.** Each role is a single loop over one event stream ([ADR-0006](docs/adr/0006-host-boundary.md)). This is what makes the end-to-end suite deterministic and traces replayable — don't propose spawning threads inside `engine`.
