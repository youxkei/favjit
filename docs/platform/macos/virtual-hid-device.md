...
heartbeats, and deadlines on both directions — not a bare write of those
...
A client has to answer a `health_check` with a `health_check_response`, and has to
keep saying something at all.

### Both ends beat, and the service's own beat is what makes this a trap

Both ends of the connection are `pqrs::unix_domain_stream::impl::peer`, so the
numbers below are the same class's on either side.

**Which numbers they are is the installed daemon's, and neither the repository's
tip nor the last version read is it.** `Info.plist` under
...
gives the version to read `src/Daemon/include/virtual_hid_device_service_server.hpp`
at, and the values it passes to `server_options` are the whole of it: whatever it
leaves out comes from `vendor/vendor/include/pqrs/unix_domain_stream/options.hpp`,
whose defaults are far stricter than the overrides. **8.0.0 through 8.4.0 pass
`max_message_size` and nothing else; 8.5.0 overrides three of them.** Installed
here on 2026-09-15 is 8.5.0, under Karabiner-Elements 16.3.0, so:

| | value | 8.5.0 | the default, where a version does not override it |
|---|---|---|---|
| `heartbeat_interval` | how often **this end sends a heartbeat at the client**, on a timer of its own | 3 s (not overridden) | 3 s |
| `read_timeout` | how long one read may wait — `read_header` starts this deadline **before** awaiting the next frame's header, so it is the bound on going quiet | 15 s | **5 s** |
| `heartbeat_timeout` | the same thing, more loosely; refreshed by any complete frame | 30 s | 10 s |
| `write_timeout` | how long one write to the client may take | 15 s | 5 s |

So the effective rule is `read_timeout`, and it is stricter than the deadline named
for heartbeats: **a client that sends nothing for that long is closed**, with
`peer_error_occurred: Operation timed out` followed by `peer_closed` in
`/var/log/karabiner/virtual_hid_device_service.log`.

**Both columns are worth building against, and the strict one is the one to hold
to.** `driver_version` is 1.8.0 and `client_protocol_version` is 7 at both 8.0.0
and 8.5.0, so nothing about what a client says changes with the version — only
how long it may go quiet. A client that beats every 3 s clears 15 s by twelve
seconds and 5 s by two, and a cadence that can slip to twice what it asked for
clears one and not the other: which of them is installed is a fact about the
machine on the day, and the smaller number is what the code is right against.

The trap is `heartbeat_interval`. The service does not wait to be beaten at — it
beats at the client every 3 s, from `start_heartbeat_timer`, which reschedules
itself and consults nothing about reads. A heartbeat it receives is not answered
(`case message_type::heartbeat: break;`); it only refreshes the deadline.

**Which means a client cannot derive its own cadence from its reads.** A read given
a 3 s bound is racing the service's 3 s timer, and what comes back is a frame far
more often than it is a timeout — so a client that beats when a read comes back
empty beats hardly ever, and while somebody is typing the reports hide it. The
connection then lasts exactly as long as the typing, and one `read_timeout` after
the person stops, the service closes it. On this machine that read as favjit dying
and restarting every 6 to 81 seconds, with the keyboards handed back each time.

The driver's own client does not have this problem because it does not do that:
`start_heartbeat_timer` is a timer, not a branch of the read loop.

**So favjit beats on a clock of its own, and its reads are bounded by what is
left of that clock.** `favjit_engine::output::serve` sends a heartbeat once
`OUTPUT_HEARTBEAT` has passed since the last one, whatever the reads did in
between, and gives each read the remainder of the interval rather than the whole
of it. The remainder and not the whole, because the bound is on one read and a
frame is however many reads it takes: given the whole interval, a read that comes
back with a frame partway through leaves the next one another whole interval, and
the beat after it goes out at twice the cadence it was asked for — which clears
15 s and does not clear 5. Posting reports is not a substitute either — a run
with nothing to type would go quiet and be dropped — and neither is counting the
health checks answered and requests acknowledged, which do hold the connection up
but only for as long as the service keeps asking.

`crates/e2e/tests/output_connection.rs` is where that is held to. The simulator's
service is the one described here rather than a list of frames: it beats every 3 s
and closes a connection it has heard nothing on for **5**, so a run that stops
beating — or whose cadence slips past that — fails there with the same
`HeardNothing` the real one showed in its log. Five and not whatever is installed
today, because a simulator that tracked the installed daemon would stop asking the
question the moment a version with more margin arrived, and the margin is not
something a run gets to depend on.

**And the recording holds it.** Every frame a run puts on this connection is a
record of which frame it was and the number the write answered
(`favjit_engine::trace::Served`), and the loop coming back is a record of which of
its ways out it took (`Lost`). Without those a reading shows a run writing reports
successfully until it stops, which is what every one of those ways out looks like
from the injections alone — the writes that hold the connection up are not
injections, so a trace of the injections says nothing about it. `favjit
--trace-report` prints the counts and the reason.
...
[ADR-0014](../../adr/0014-convert-the-function-row-to-its-icons.md); why it has to
...
## The service closes the connection on every wake from sleep

Read off `/var/log/karabiner/virtual_hid_device_service.log` beside `pmset -g log`
on 2026-09-11 (macOS 26.6.2), with favjit installed as the daemon and beating every
3 s. Every `peer_error_occurred (...): Operation timed out` / `peer_closed` pair in
the service's log that day fell within a second of a wake the power log records —
`DarkWake from Deep Idle`, `Wake from Deep Idle`, `DarkWake to FullWake` — where the
sleep before it had lasted at least the `read_timeout`:

| service closed | power log |
|---|---|
| 14:23:00 | 14:23:01 DarkWake |
| 14:25:49 | 14:25:49 DarkWake |
| 14:29:41 | 14:29:41 DarkWake |
| 15:30:46 | 15:30:43 DarkWake |
| 15:32:04 | 15:32:04 DarkWake |
| 15:32:36 | 15:32:36 Wake |
| 16:22:42 | 16:22:42 DarkWake to FullWake |

With the lid closed the machine cycles `Maintenance Sleep` → `DarkWake` every 15 to
60 s, and the connection was closed on each of those wakes in turn (11 times between
14:13 and 14:30). So the client's own beat cannot hold the connection across a sleep:
nothing runs while the machine is asleep, and the service's deadline fires at the
wake. What a client sees is the connection going after a sleep, exactly as if it had
stopped beating.

favjit's answer is the one already in place for a connection that goes: the run ends
with `OutputGone` and is started again by launchd, which reconnects. A client that
wanted to stay up across sleep would have to reconnect inside the run instead.

Three closes that day had no wake within a minute of them (16:01:09, 16:05:37,
16:12:22), and the one at 16:12:22 is measured further:

- The last frame favjit put on the connection was therefore at about 16:12:07.9.
- The kernel went on counting input the whole time: 51 `requestUserActive` tickles
  from the seized Bluetooth keyboard between 16:12:10 and 16:12:19, so somebody was
  typing on a keyboard this run held and none of it became a frame.
- The run was not wedged. The watchdog's allowance is 2 s and it said nothing, so
  the loop kept coming round — and it comes round on the supervisor's probes, which
  arrive on the same stream the keystrokes do.
- The run ended 0.4 s after the close, on the connection having gone.

So a run can hold the keyboards, keep answering its supervisor, and convert nothing,
for as long as the service takes to give up on it. Liveness here is the loop coming
round, and that is not the same fact as input arriving.

### A close with no wake is the client's cadence slipping past five seconds

Closes with no wake near them are the same `read_timeout` as the ones on a wake,
reached the other way: by the client's own beat being late rather than by nothing
running at all. Read off the service log beside `pmset -g log` on 2026-09-14
(macOS 26.6.2, build 25G83, Darwin 25.6.0), with the daemon at 8.0.0 and so on
the 5 s default above.

Two that morning, 10:57:56.896 and 11:23:47.703, have no sleep or wake between
10:55:37 and 11:27 — the machine was awake throughout — beside one at 10:55:35.954
that does fall on a `Wake from Deep Idle`, so both routes to it appear within the
same two hours. The recording of the 11:23:47.703 one survives (1549.527 s of
span, nothing dropped from the start, so it covers the whole run from
`peer_connected`):

- 511 beats, 3.0 s apart, every write answering 0 — the last four at 1535.09,
  1538.21, 1541.35 and 1544.19 s into the run.
- The run read the close at 1549.2 s, **5.0 s after that last beat**.
- Nobody typed for the final 59 s: the last key event is at 1490.0 s, and every
  record after the last beat is a supervisor probe.

Three recordings taken on that machine that day put the last beat 5.0, 5.0 and
5.1 s before the close, and one gap earlier in a run measured 6.08 s. A client
that reads with a bound of its whole beat interval and then asks whether a beat
is due comes round one interval late, so its cadence runs at up to twice what it
asked for — 3 s asked for is 6 s delivered, and the deadline is 5.

The afternoon shows what that costs: closes at 17:13:26, 17:13:53, 17:14:05,
17:14:11, 17:14:56, 17:16:32, 17:17:07, 17:17:20 and 17:18:07, with no sleep or
wake in the power log at all, and connections living 6.0, 11.0, 12.0, 26, 41, 47
and 95 s. **A 6.0 s connection cannot be a client that went quiet for 15**, which
is what says the deadline in force is the default and not the override.

The margin is what separates the two, and swapping the daemon under an unchanged
client shows it: the same binary that had been closed roughly every fifteen
minutes through the night of 2026-09-14 into 08:06 the next morning — around
thirty closes, every one `Operation timed out` — went on under 8.5.0, installed
at 08:15 on 2026-09-15 with Karabiner-Elements 16.3.0, with no close at all in
the minutes after. A cadence that slips to six seconds clears fifteen and does
not clear five.
...
- Whether the three closes on 2026-09-11 that no wake explains (16:01:09,
  16:05:37, 16:12:22) are the same slipped cadence. No recording of them survives,
  and the one at 16:12:22 has input arriving that became no frame, which the slip
