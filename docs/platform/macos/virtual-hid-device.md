...
numbers below are the same class's on either side. The service
(`src/Daemon/include/virtual_hid_device_service_server.hpp`) takes the library's
defaults and overrides three of them:

| | value | what it bounds |
|---|---|---|
| `heartbeat_interval` | 3 s (the default, not overridden) | how often **this end sends a heartbeat at the client**, on a timer of its own |
| `read_timeout` | 15 s | how long one read may wait — `read_header` starts this deadline **before** awaiting the next frame's header, so it is the bound on going quiet |
| `heartbeat_timeout` | 30 s | the same thing, more loosely; refreshed by any complete frame |
| `write_timeout` | 15 s | how long one write to the client may take |

So the effective rule is `read_timeout`, and it is stricter than the deadline named
for heartbeats: **a client that sends nothing for 15 s is closed**, with
...
connection then lasts exactly as long as the typing, and 15 s after the person
stops, the service closes it. On this machine that read as favjit dying and
restarting every 6 to 81 seconds, with the keyboards handed back each time.
...
**So favjit beats on a clock of its own.** `favjit_engine::output::serve` reads
`OutputHost::now` and sends a heartbeat once `OUTPUT_HEARTBEAT` has passed since
the last one, whatever the reads did in between; a read that came back empty asks
for nothing. Posting reports is not a substitute either — a run with nothing to
type would go quiet and be dropped — and neither is counting the health checks
answered and requests acknowledged, which do hold the connection up but only for
as long as the service keeps asking.
...
and closes a connection it has heard nothing on for 15, so a run that stops beating
fails there with the same `HeardNothing` the real one showed in its log.
...
sleep before it had lasted at least the 15 s `read_timeout`:
...
16:12:22). What went quiet for 15 s there is not established, and the one at
16:12:22 is measured further:
...
round, and that is not the same fact as input arriving. What stopped arriving, and
whether the output thread stopped beating for the same reason or a different one,
needs the recording of such a run; the traces of these are gone, since the ring is
the supervisor's memory and the restart takes it.
...
- What silenced the connection for 15 s at the three closes above that no wake
  explains.
