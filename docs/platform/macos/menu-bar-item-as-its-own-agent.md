# Turn converting off from a menu bar item that is its own binary and its own launchd job

- **Date**: 2026-08-29

The finding it rests on is [menu bar status items](menu-bar-status-items.md), and
the file it turns off with is [the daemon install's](install-as-a-daemon-and-turn-off-with-a-file.md).

## Context

The failure the supervisor does not cover is a favjit that is alive and converting
wrongly. It keeps heartbeating, so it is never killed ([ADR-0008](../../adr/0008-input-suppression-and-watchdog.md)),
...
[The daemon install](install-as-a-daemon-and-turn-off-with-a-file.md) settled what
...
the workspace, and only into this binary. `engine` and the converter are unaffected,
which is what [ADR-0005](../../adr/0005-crate-layout.md)'s layout is for. They also bring the
