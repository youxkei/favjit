...
## The capture thread stalled for over 4 s at two starts, with the seize held

Observed 2026-09-11 (macOS 26.6.2) from the installed daemon's logs, both times a
few seconds after the run before it had ended on the output connection closing:

| run started | devices opened with seize (kernel `IOHIDFamily` log) | watchdog |
|---|---|---|
| 16:12:24 | 16:12:25.3 `IOHIDUserDevice` (the Bluetooth keyboard), 16:12:25.5 `AppleHIDTransportHIDDevice` (built-in) | `no heartbeat for 4.18s` at 16:12:30 |
| 16:15:46 | none logged before the kill | `no heartbeat for 4.30s` at 16:15:50 |

In favjit the watchdog's probes are read by the thread that makes the IOKit calls,
so a probe unanswered says that thread was inside one of them — and the run logged
`asked to stop` and exited within 0.4 s of the `SIGTERM` both times, so the run's
own loop was not the thread that had stopped. In the first case the seize on both
keyboards had succeeded; whatever the thread stalled in came after the queues were
set up, and the built-in and Bluetooth keyboards were dead for the 5 s until the
kill, which is the failure ADR-0008 bounds rather than prevents. `bluetoothd` logged
`Adding LE Connection` 1 s before the first start and an advertising event 1 s after
the second; whether the Bluetooth keyboard reconnecting is what the IOKit call waited
on is not established. The trace of neither run survived the restart.

**Nothing held any device for the 196 s between those two runs.** Every
`IOHIDLibUserClient` open the kernel logged from 16:12:31 to 16:16:06 carries `(0x0)`
— the system's own driver taking the Bluetooth keyboard back when it reconnected at
16:14:35 — and no `(0x1)` at all. The keyboards were the machine's own and
unconverted through that gap, which is the direction ADR-0008 asks for.

`requestUserActive` in the kernel log is the instrument for whether a device
produced anything at all, and it catches both this machine's keyboards
(`AppleHIDTransportHIDDevice:0x100000fcc` built in, `IOHIDUserDevice` for the
Bluetooth one) **whether or not they are seized**: the tickles above arrived from a
keyboard a run had seized. It fired 51 times in the ten seconds before the first
stall and not once from 16:12:20 until 16:17:00, so nothing was pressed or touched
across that window — a stretch with nothing held cannot be read as a stretch that
refused input. Whether the built-in **trackpad** tickles has not been established:
it never appears in these lines, so its silence says nothing either way.
