# Send output through a virtual HID device rather than synthesised events

- **Date**: 2026-08-28

What carries a converted keystroke or a relayed pointer movement to the rest of
this system. [ADR-0003](../../adr/0003-unified-conversion-pipeline.md)'s single
pipeline is unaffected — this is what the end of that pipeline writes to. The
findings it rests on are [the virtual HID device service](virtual-hid-device.md)
and [event injection](event-injection.md).
...
stays a seize ([ADR-0008](../../adr/0008-input-suppression-and-watchdog.md)).
...
([ADR-0005](../../adr/0005-crate-layout.md)'s host is where that exclusion lives).
...
favjit itself stays a plain binary, so [ADR-0001](../../adr/0001-use-rust.md)'s distribution
...
with it. Each step of that protocol — initialisation, health checks, heartbeats and
readiness — is one host operation, while `engine` orders the sequence
([ADR-0006](../../adr/0006-host-boundary.md)).
...
which is the direction [ADR-0008](../../adr/0008-input-suppression-and-watchdog.md) asks for.
...
distribution [ADR-0001](../../adr/0001-use-rust.md) rests on, in exchange for a device
