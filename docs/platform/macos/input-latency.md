...
## A drain made after the slice waits for the slice

Measured on macOS 26.6.2 (build 25G83, Darwin 25.6.0, arm64 T6050), 2026-09-09,
from the installed daemon, both keyboards seized, read off the trace the
supervisor keeps (`favjit --trace-out` then `--replay`, ADR-0009): 583 s of use,
7,470 events, of which ~50,300 are consecutive TrackPoint pointer reports on one
device.

The value-available callback puts the queue down and nothing more; the values are
read off it by the loop that turns the run loop, after `CFRunLoopRunInMode`
returns. With the third argument `false` — keep running until the 50 ms slice
expires — the gaps between one pointer report's stamp and the next were:

| gap between consecutive reports | count |
|---|---|
| under 100 µs | 46,591 |
| 40–60 ms | 102 |
| over 60 ms | 3,643 |
...
That is, the reports arrive in bursts — 14 to 27 in a burst is the common size,
once per slice or longer — where a TrackPoint sends one about every 8 ms. What a
person feels is the pointer moving in steps and every key waiting up to a slice.

The finding above ("a value arriving mid-slice wakes the loop from inside
`CFRunLoopRunInMode` and the callback runs then") is still true and is not what
matters here: the callback runs at once, and what it does at once is put the
queue down. The read happens where the run loop's turn comes back, and with
`false` that is the end of the slice.

### What favjit does about it

The third argument is `true`: come back as soon as one source has been handled,
which is the callback having put a queue down. The loop then drains what was put
down and turns again. A turn in which nothing arrives still waits the whole
slice, which is what the probe check between turns relies on.

Re-measured the same way with `true`, 116 s of use, 4,940 events, ~31,000
consecutive pointer reports on the same device:
...
The bursts are two reports in 12,089 cases and three to six in the rest, with
10–40 ms between bursts. The 40 ms-and-longer gaps are down from 3,745 to 304 and
the burst sizes from 14–27 to 2. What the pairs are — two reports the TrackPoint
Keyboard II's Bluetooth link delivers per connection event, or two that arrive
between one drain and the next — was not separated here; either way the drain is
no longer what spaces them.
