# Let the OS produce the key repeats

- **Date**: 2026-08-29

Follows from [the output path](output-through-a-virtual-hid-device.md): output here
is a device that holds key state, and that changes who repeats a held key. The
finding it rests on is [key repeat](key-repeat.md).

## Context

[ADR-0010](../../adr/0010-clock-on-the-event-with-a-timer.md) has a timer on the host surface,
