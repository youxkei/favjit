# Install as a launchd daemon and turn favjit off with a file

- **Date**: 2026-08-29

How this system's favjit is started and stopped, alongside
[what its output goes through](output-through-a-virtual-hid-device.md).
...
would be the failure [ADR-0008](../../adr/0008-input-suppression-and-watchdog.md) rules out.
...
That throttle is not the whole of the gap. Observed 2026-09-11 (macOS 26.6.2) in
launchd's own log (`/usr/bin/log show --predicate 'process == "launchd"'`): after
the watchdog ended a run that had lived 6 s, launchd wrote `Service only ran for 6
seconds. Pushing respawn out by 4 seconds` and `service spawn deferred by 4 seconds
due to throttle` at 16:12:30 — and spawned the job at 16:15:46, 196 s later, with
no sleep in the power log between. In that gap the keyboards were the machine's own
and unconverted. What stretched a 4 s deferral to 196 s is not established.

Installing needs a second thing installed first: the DriverKit package
[the output path](output-through-a-virtual-hid-device.md) depends on. Nothing here
sets that up, and favjit refuses to start rather than seizing with nowhere to send
keystrokes.
