...
[ADR-0014](../../adr/0014-convert-the-function-row-to-its-icons.md): the row has to
be converted, because a function key handed back through a virtual device is a
function key.

## A null queue is not a queue with nothing on it

Observed on macOS 26.6.2 (build 25G83, Darwin 25.6.0, arm64 T6050), 2026-09-09,
from an unsigned `rustc`-built binary linking only `IOKit`:

```
IOHIDQueueCopyNextValueWithTimeout(NULL, 0.0)   -> SIGSEGV (exit 139)
```

It does not answer `NULL`, and it does not answer an error. So a read of a queue
cannot be made unconditionally with nothing where a queue would be — which is
what a run has to be able to do for the read to be one call with no question in
front of it (ADR-0006).

### What favjit does about it

`favjit_host::capture::CaptureHost::next_thing_with_values` hands the queue over
as a `favjit_host::Handed` before anything is read off it, and
`next_value_of` takes that handle. A drain that has no queue makes no read at
all, because the run only reaches the read once a handle has come back.

The same shape is used wherever a handle would otherwise have to be null: the
device the run gives back, the array a queue is filled from, the place in that
array an element sits at. None of those was probed with a null — the one probe
above was enough to settle that IOKit's own objects are not null-tolerant, and
each is reached through a handle for the same reason.
