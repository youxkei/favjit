...
**A function that reaches the machine takes no turnings, and every turning a host needs lives in a function that reaches nothing.** There are two shapes and no third: one is a single call with the whole of the body being the argument going out and the answer coming back, and the other turns a value into another value and touches no device. A turning standing beside the call is what a decision looks like when it is written as plumbing, and the one that is genuinely a conversion — a report's own request number, a code read as whether the write went — is separable from the call by construction: lifting it out is what shows it was one.

**The conversions are registered, and that is what makes them readable.** Each is named with what it turns into what, in one list. A list of conversions is a list a person can disagree with — turning a page and a usage into the platform's own spelling reads as a conversion, and choosing which of two homes a file sits under does not — where a list of functions being let off the rule says only that somebody once decided not to look. And an entry cannot excuse a function that reaches the machine, because the two shapes are checked against each other: a registered conversion making a platform call fails as loudly as a call with a turning beside it.
...
- **What a missing dependency cannot refuse, a check over the hosts' own syntax refuses.** It runs in the suite rather than beside it, so a `cargo test` that passes is a claim about the boundary and not only about behaviour, and it fails on a message a host writes through `std`, on a bound or a limit a host states for itself, on a `#[test]` outside the inventory below, and on the shape of every function in a host.
- **The two shapes are checked against each other, and not only by counting calls.** A count is blind to the halves of that body that make no call, so what is refused is the shape: a turning taken beside a platform call, a turning taken by a function no conversion list names, a registered conversion that reaches the machine, and more than one call in one body. The four cannot all be satisfied by moving code around inside a host — a turning has to leave the function that calls, and where it lands has to say what it converts — which is the point: what a reader ends up with is the conversions, written down.
- **Which calls reach the machine is declared, not inferred.** Core Foundation's own value operations and IOKit's accessors over a value it has already handed over ask no device anything: what those answer was in the value before the call was made. Nothing in the shape of a call says so, and a body reading a `CFTypeRef` through four of them is one question of the machine and the conversions around it — so the ones that ask nothing are named in one list beside the ones that do.
- **A host operation that keeps what a call answered** — a counter, a flag, a list added to — has taken the fact the run was owed and made it state of its own, which the run then has to ask for a second time. Holding a handle a call returned is not that, and is the shape of a host: the two are told apart by the value written rather than by the field it goes in.
- **What is left over is a list in one place, and an entry names which of three things it is.** The thread a host turns a loop on, a frame off a stream socket, or the answer a procedure the platform calls has to give on that call. None has an `engine` to move to: a thread does not exist until the call that makes it, so there is no order for `engine` to keep from outside it; the framing is the transport's own format, and moving the reads up would leave the writes spelling it here; and a hook's answer is its return value, on the platform's own call, on a thread nothing above the boundary can be reached from. So the list does not shrink to nothing, and what keeps it honest is that an entry naming none of the three reads as wrong. It is one list in one place either way, because a rule whose exceptions are spread through the code it governs is a rule nobody can total up.
...
**That covers the loop that reads the keyboards, and it is the one where it matters most.** A platform ties reading to a thread — a device's queue delivers onto that thread's run loop, a low-level hook is called on the thread that installed it — so the temptation is to write the whole of it there, and what ends up written is the order the setup takes, the order of the drains inside the turn, a byte count turned into that many events, and a dispatch. Each of those is a decision, and the argument for the one that matters is not obvious: asking to be told about arrivals **before** listing what is already attached is what keeps a keyboard plugged in between the two from being missed, at the price of finding one twice — which is only safe because the second finding is recognised by the platform's own id for the device and dropped. An argument like that written in a comment is held by whoever last read it. Written as a loop `engine` turns, it is held by a test.
