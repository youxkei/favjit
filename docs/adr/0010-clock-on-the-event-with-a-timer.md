...
[ADR-0009](0009-trace-and-replay.md) sets the constraint over both. `engine` is a deterministic function of its event stream, and **anything `engine` reads that is absent from the trace makes a replay diverge**. A `now()` return value is exactly such a read: keeping replay honest would mean recording the call and its result, which is the same information an event timestamp already carries.
...
- Replay needs no extra machinery: a recorded trace already holds every timestamp `engine` ever saw, and a wake-up is an event in it like any other.
...
Rejected: a value `engine` read that no event carries is exactly what ADR-0009 rules out. Handing them in as configuration puts them where the layout is, which is also what a recorded trace needs to replay.
