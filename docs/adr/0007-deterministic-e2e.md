...
Everything impure is already behind the host boundary, and `engine` holds no concurrency of its own ([ADR-0006](0006-host-boundary.md)), so a substitute host is enough to place the whole system in any of those states.
...
Unnecessary. With no concurrency inside `engine` there are no preemption points to schedule; an ordered event queue covers everything that remains.
