# ADR-0012: Carry input over a Noise session on TCP, with the sink listening
...
running as a supervised daemon that is always up ([the sink's install](../platform/macos/install-as-a-daemon-and-turn-off-with-a-file.md)).
...
**What both machines agree on is stated once, in `engine::link`**, beside the frame it
...
**The handshake and the session it produces are `engine::noise`'s, driven a step at a
time by `engine::link::serve`** — construct the responder, open the source's first
message and produce the answer, learn the session and the key it proves. A host reads
and writes the bytes each step needs and supplies the ephemeral key's entropy;
everything decided from what comes back is `serve`'s, down to a sealed record being
opened separately from being taken. Then the list is read, and only a peer that is in
it has a frame read from it.

**Bringing the link up is the run's, not the host's.** `engine::sink::run` establishes
the identity this machine presents over the three file operations of
`engine::pairing::IdentityStore`, binds the socket with it, and hands the machine the
...
**A connection that could not be taken is not the socket failing, and `engine` is what
...
the count back to zero. A count rather than a delay, because `engine` has no clock
...
into the stream, and `engine` decides what that means: the run ends, having converted
what already arrived.

**Pairing is advertised under a name of its own**, `engine::link::PAIRING`, at a port of
...
The socket is a host concern on both sides, and lives in `host-windows` and
`host-macos` ([ADR-0006](0006-host-boundary.md)). The cryptography they perform is
reading and writing bytes and supplying entropy — the construction itself is
`engine`'s, so the end-to-end suite can drive a source through a simulated link into a
sink without a network.
...
([the sink's install](../platform/macos/install-as-a-daemon-and-turn-off-with-a-file.md)
accepts the same trade for the on/off switch).
...
host reaches the same `engine::noise` construction rather than one of its own, so the
two ends cannot disagree about it.

It adds a cryptography dependency. `snow` implements Noise in Rust with no async
runtime, which keeps it inside the shape ADR-0006 asks for: no concurrency in `engine`,
and hosts that a single loop drives. `engine` holds the cryptography and needs only
entropy, which it takes from the host; what a replayed trace carries is the events the
handshake let through, not the handshake itself, so the entropy a resumed run's
handshake draws does not have to be the one that made the original session
...
### Each host driving its own copy of the construction

A host builds its own `HandshakeState` and calls `answer`/`open` on it, with `serve`
only told whether each step succeeded. Not taken: the sequence over the steps —
constructing the responder, learning the peer, sealing what goes out next — is then a
host's, and a host that decides anything decides it where nothing can drive it
([ADR-0006](0006-host-boundary.md)). It also stops being one construction: a second
host reaching its own copy of it is the very risk stating it once in `engine` exists
to remove.
