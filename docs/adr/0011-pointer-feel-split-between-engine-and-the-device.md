# ADR-0011: Turn the wheel over in `engine` and set the speed on the output device
...
takes its pointer with it ([the sink's output path](../platform/macos/output-through-a-virtual-hid-device.md)).
...
`engine` inverts the wheel, as part of the one conversion pipeline
