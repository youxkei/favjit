...
#[cfg(windows)]
pub use region::{on_handle as map_trace_region, passed_down as trace_handle, Region};
#[cfg(windows)]
pub use supervisor::{do_not_wait, named_handle as supervisor_handle, Supervisor};
...
mod host {
    ...
    pub struct WindowsHost {
        ...
        clock: Clock,
        /// Where favjit's own files live, handed in at the start.
        ///
        /// Held rather than read where a file is wanted, because reading it is
        /// two calls into the machine and a choice between what they answered:
        /// a host asking for it beside the call that uses the path would be
        /// making three where it may make one (ADR-0006).
    ...
    impl WindowsHost {
        ...
        /// to.
        /// The directory is handed over for the same reason: it is two reads of
        /// the environment and which of them to use, and a host that composed it
        /// here would be making those calls beside the one this makes.
        /// The moment the run's bound is measured against is handed over on the
        /// same ground: reading the wall clock here, beside the clock the stamps
        /// are on, would be two reads of a clock in one body.
        pub fn start(
            chord: Chord,
            has_a_sink: bool,
            link: Link,
            ...
            Self {
                ...
                link,
                has_a_sink,
                identity: IdentityFile::new(crate::link::identity_path(&directory)),
                directory,
                until,
                bounded: false,
                supervisor,
    ...
    impl SourceHost for WindowsHost {
        fn pinned_sink(&mut self) -> Option<String> {
            crate::link::read_sink(&crate::link::sink_path(&self.directory))
        }
        ...
        fn show_here(&mut self, strokes: &[favjit_host::source::Stroke]) {
            suppress::show_here(strokes);
        }
