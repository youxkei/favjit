...
pub fn passed_down(variable: &str) -> Option<usize> {
    a_handle(std::env::var(variable))
}

/// The handle a variable holds, and none where it holds anything else.
///
...
/// (ADR-0006).
fn a_handle(value: Result<String, std::env::VarError>) -> Option<usize> {
    value.ok().and_then(|handle| handle.parse().ok())
}
