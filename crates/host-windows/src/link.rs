...
/// Read one at a time and put together by whoever wants it, because each read is
/// a call into the machine and a host makes one of those per operation: the two
/// are [`local_app_data`] and [`user_profile`], and [`under`] is what they mean
/// together (ADR-0006). Whoever composed it hands it to whatever needs a file
/// inside it, so nothing here reads the environment twice.
/// Which of the two the directory sits under, and where it sits.
///
/// Apart from the reads rather than spelled between them, because each read is
/// a call into the machine and a host makes one of those per operation: which of
/// the two answers is used, and what is joined onto it, is a decision and lives
/// in one place that reaches nothing (ADR-0006).
///
/// `LOCALAPPDATA` first and `USERPROFILE` behind it, because the first names the
/// directory itself and the second names the profile it is inside — so the one
/// that needs no spelling added is the one to prefer.
pub fn under(local: Option<PathBuf>, profile: Option<PathBuf>) -> PathBuf {
    local
        .or_else(|| profile.map(|home| home.join(r"AppData\Local")))
        .unwrap_or_default()
        .join("favjit")
}
...
pub fn identity_path(directory: &std::path::Path) -> PathBuf {
...
pub fn sink_path(directory: &std::path::Path) -> PathBuf {
    directory.join("sink")
}
