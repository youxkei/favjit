...
/// (`favjit_hid::scancode` reads a key out of those three).
...
/// `VK_MENU`, which is either alt key — the modifier the chord is made with.
...
extern "system" {
    ...
    pub fn SetPropW(window: Handle, name: *const u16, data: Handle) -> i32;
    pub fn GetPropW(window: Handle, name: *const u16) -> Handle;
...
/// `INPUT`, as the one kind favjit sends.
///
/// The keyboard arm written out rather than the union: the mouse and hardware
/// arms are larger, so a structure holding only this one would be too small for
/// the size `SendInput` is told — which is why the tail is here, padding the
/// keyboard arm out to the union's own size, and why [`tests`] measures it.
#[repr(C)]
pub struct Input {
    pub kind: u32,
    pub padding: u32,
    pub key: KeyboardInput,
    pub rest_of_the_union: [u8; 8],
}

/// `KEYBDINPUT`, the arm above.
#[repr(C)]
...
/// `INPUT_KEYBOARD`, and the flag that makes one of them a release.
pub const INPUT_KEYBOARD: u32 = 1;
pub const KEYEVENTF_KEYUP: u32 = 0x0002;

/// The virtual key a refused chord's modifier is masked with.
///
/// `VK_CONTROL`. Control rather than a key that types something, because what
/// this has to be is a key the machine acts on in no way when it arrives on its
/// own: a letter would be typed into whatever has the foreground, and the
/// modifier keys are the ones a tap of does nothing at all
...
mod tests {
    ...
    fn the_structures_are_the_size_the_headers_say() {
        ...
        // The one this crate fills in rather than reads: too small and
        // `SendInput` is handed a size the array does not have.
