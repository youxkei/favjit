...
pub struct Pairing {
    ...
    address: SocketAddr,
    /// Where favjit's own files live, handed in for
    /// [`crate::WindowsHost`]'s reason.
    directory: std::path::PathBuf,
}

impl Pairing {
    pub fn new(address: SocketAddr, directory: std::path::PathBuf) -> Self {
...
impl SourcePairingHost for Pairing {
    ...
    fn make_sink_directory(&mut self) -> Result<(), Trouble> {
        let path = crate::link::sink_path(&self.directory);
        crate::link::made(
