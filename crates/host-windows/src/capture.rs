...
/// Start reading, and say whether the thread it happens on is there.
///
/// The clock is handed in rather than taken here, because taking it and starting
/// the thread are two calls into the machine and a host makes one of those per
/// operation: which of them happens first is the order a run drives (ADR-0006),
/// and it matters — a clock taken after the thread began would be behind the
/// first keystroke stamped against it.
///
/// Whether the thread started is the one failure visible from here: everything
/// it does after that — the window, the registration — fails on the thread, and
/// shows up as a stream that ended.
pub fn capture(
    chord: Chord,
    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            watch(
                chord, clock, events, supervisor, probe_tick, raw_bytes, work,
            )
        });
    started(spawned)
}
...
impl favjit_host::capture::SourceCapture for Turning {
    ...
    fn what_is_waiting(&mut self) -> Option<favjit_host::capture::Waiting> {
        the_front(&self.watching.arrived, &self.watching.devices)
    }

    fn take_what_is_waiting(&mut self) -> Option<EventKind> {
        taken(&mut self.watching.arrived, &self.watching.devices)
    }
}
...
fn what_else_it_said(message: &Msg) -> Option<(Handle, Said)> {
    match message.message {
        ...
        // Read out of the word through the crate both ends of the ask share, and
        // not through a name of this host's own: a second spelling of "which
        // machine this number means" is a spelling that can come to disagree
        // with the one the tray item wrote it with.
        favjit_tray_wire::WM_ASKED => favjit_tray_wire::from_ask(message.wparam)
...
/// The front of what the turn put down, as the two facts the run reads it by.
fn the_front(
    arrived: &VecDeque<Arrived>,
    devices: &[Device],
) -> Option<favjit_host::capture::Waiting> {
    arrived.front().map(|one| favjit_host::capture::Waiting {
        from_a_device: named_device(&one.said),
        numbered: numbered(devices, one.at),
    })
}

/// The front of what the turn put down, taken, as the event the run reads.
fn taken(arrived: &mut VecDeque<Arrived>, devices: &[Device]) -> Option<EventKind> {
    arrived
        .pop_front()
        .map(|one| as_an_event(one.said, numbered(devices, one.at)))
}
...
fn as_an_event(said: Said, device: Option<DeviceId>) -> EventKind {
    // The run reads the front before it takes it, so what came from a device it
    // has not numbered is never taken and this stands for nothing a report is
    // ever built with.
