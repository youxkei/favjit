...
///
/// The clock's origin is taken by [`Searching::start_clock`] and nowhere else,
/// because taking it is a call into the machine and a host makes one of those
/// per operation: an origin taken where the socket is opened would be a second
/// call beside that one, and it is the run that decides when a search's clock
/// starts (ADR-0006).
///
/// Absent until then, and read through [`origin`]: what a reading before the
/// clock was started is relative to is the epoch, which is what a search that
/// has not begun has elapsed from.
pub struct Search {
    socket: UdpSocket,
...
impl favjit_host::Searching for Search {
    ...
    fn start_clock(&mut self) -> Instant {
        self.origin = Some(std::time::Instant::now());
        Instant::default()
    }

    fn now(&mut self) -> Instant {
        since(origin(self.origin).elapsed())
    }
...
/// The origin a search's clock is read against, and this moment where its clock
/// has not been started.
///
/// This moment rather than a reading that says so, because there is nothing to
/// say it to: a search reads the clock only after the run has started it, and an
/// origin of now makes the one reading that could come first read as no time
/// having passed.
fn origin(taken: Option<std::time::Instant>) -> std::time::Instant {
    taken.unwrap_or_else(std::time::Instant::now)
}
...
/// The machine four bytes and a port name, with nothing asked of a resolver.
pub fn at(address: [u8; 4], port: u16) -> Option<SocketAddr> {
    Some(SocketAddr::from((Ipv4Addr::from(address), port)))
}
...
pub fn looked_up(host: &str, port: u16) -> Option<SocketAddr> {
