...
use favjit_host::source::{Answering, Answers, Arrived, Shown, Stroke, Suppressing};
...
impl Answers for Called {
    ...
    /// A control tap, sent from inside the procedure the tap will be delivered
    /// to.
    ///
    /// Sound because it comes back: `SendInput` from a hook procedure re-enters
    /// this one, and what arrives carries `LLKHF_INJECTED`, which
    /// [`a_key`] reads as nothing of ours — so the tap is neither relayed nor
    /// masked again, and the recursion is one call deep.
    fn show_here(&self, strokes: &[Stroke]) {
        show_here(strokes)
    }
}

/// Show this machine these strokes, in this order.
///
/// One call whatever the run asked for, because what a machine makes of a key
/// depends on what came before it: strokes handed over one at a time would be
/// this deciding whether to wait for the next (ADR-0006).
///
/// Sound from inside a hook procedure: what goes out comes back through this
/// process's own hooks and is dropped there for carrying `LLKHF_INJECTED`, which
/// [`a_key`] reads as nothing of ours — so a stroke is neither relayed nor
/// answered again, and the recursion is one call deep.
pub fn show_here(strokes: &[Stroke]) {
    let inputs = as_inputs(strokes);
    unsafe {
        SendInput(
            inputs.len() as u32,
            inputs.as_ptr(),
...
fn as_inputs(strokes: &[Stroke]) -> Vec<Input> {
    strokes.iter().map(one_input).collect()
}

/// One of them, under the virtual key this machine calls that one and the flag
/// that makes a stroke a release.
fn one_input(stroke: &Stroke) -> Input {
    Input {
        kind: INPUT_KEYBOARD,
        padding: 0,
        key: KeyboardInput {
            vkey: a_virtual_key(stroke.key),
            scan: 0,
            flags: going(stroke.released),
            time: 0,
            extra: 0,
        },
        rest_of_the_union: [0; 8],
    }
}

/// Which key this machine calls each of the two a run can name.
///
/// `Shown` is `#[non_exhaustive]`: a key this crate has no name for yet is shown
/// as the one it acts on in no way, which is the safe half of the pair — a
/// stroke that does nothing rather than a modifier left down.
fn a_virtual_key(key: Shown) -> u16 {
    match key {
        Shown::TheModifier => VK_MENU as u16,
        _ => VK_CONTROL,
    }
}

/// Which half of a stroke this is, as the flag the call takes.
fn going(released: bool) -> u32 {
    match released {
