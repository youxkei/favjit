...
pub fn named_handle(name: &str) -> usize {
    a_handle(std::env::var(name))
}

/// The handle a variable holds, and zero where it holds anything else.
///
...
/// number (ADR-0006).
fn a_handle(value: Result<String, std::env::VarError>) -> usize {
    value
        .ok()
...
///
/// Asked for apart from the handle rather than folded into reading it, because
/// both are calls into the machine and a host makes one of those per operation:
/// which of them happens first is the order a run drives (ADR-0006).
pub fn do_not_wait(handle: usize) -> usize {
...
/// is zero in each, and which handles these are is read by whoever names them
/// (ADR-0006).
...
impl Supervisor {
    /// Whether either end is a handle somebody handed over.
    ///
    /// Zero is what Windows leaves where one was not inherited, and reading it
    /// that way is what makes a run with no watchdog need no case of its own in
    /// the calls below.
