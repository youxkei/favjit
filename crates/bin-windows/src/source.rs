...
use favjit_host_windows::{
    do_not_wait, link, supervisor_handle, tray, what_is_attached, Chord, Supervisor, WindowsHost,
};
...
pub fn main() {
    ...
    let mut host = WindowsHost::start(
        chord,
        has_a_sink,
        link,
        supervisor(),
    ...
    let mut region = match favjit_host_windows::trace_handle(favjit_engine::supervision::TRACE) {
        ...
        Some(handle) => match favjit_host_windows::map_trace_region(
            ...
            Err(error) => {
                // The sentence composed here and not in the host, because what a
                // machine's own account reads like inside one is not the
                // machine's to say (`favjit_host::Trouble`).
                error!("no trace, so nothing to read a failure off: the region on handle {handle}: {error}");
                None
            }
        },
    };
...
/// each then stopped from waiting.
///
/// Sequenced here rather than inside the host: reading the name the handle came
/// under and telling the pipe not to wait are two calls, and a host makes one of
/// those per operation (ADR-0006) — so the order is this program's and the four
/// calls are asked for one at a time.
///
...
fn supervisor() -> Supervisor {
    Supervisor {
        probe: do_not_wait(supervisor_handle(PROBE)),
        heartbeat: do_not_wait(supervisor_handle(HEARTBEAT)),
    }
}

/// Where favjit's own files live.
///
/// The two reads sequenced here rather than behind one name in the host,
/// because each is a call into the machine and a host makes one of those per
/// operation (ADR-0006) — so the order and the choice between what they
/// answered belong to a program.
fn favjit_directory() -> std::path::PathBuf {
    link::under(link::local_app_data(), link::user_profile())
}
...
fn pair(digits: &str, args: &[String]) -> i32 {
    ...
    let mut identity_store = link::IdentityFile::new(link::identity_path(&favjit_directory()));
    ...
    let mut host = favjit_host_windows::pairing::Pairing::new(address, favjit_directory());
