...
use favjit_engine::sink::{self, Bound, Ending, InputConfig, Replayed, Request, Settings};
use favjit_engine::supervision::{HEARTBEAT, PROBE};
use favjit_engine::{pointer::Tuning, trace::Trace, DeviceMatch, Layout};
use favjit_hid::Wanted;
use favjit_host::sink::SinkHost;
use favjit_host::DeviceId;
use favjit_host_macos::{
    ax_trusted, ax_trusted_asking, control, do_not_block, hid_access, hid_systems, link,
    request_hid_access, supervisor_descriptor, DryRun, HidAccess, MacOsHost, Supervisor,
    VIRTUAL_KEYBOARD_PRODUCT, VIRTUAL_KEYBOARD_VENDOR,
};
...
fn main() {
    ...
    if seconds.is_none() && asked_to_wedge(&args) {
        warn!("--wedge does nothing without --seconds: it is what should happen at that deadline");
    }

    let settings = Settings {
        repeat,
        pointer: pointer_tuning(&args),
        // How long, and never the moment it ends: placing it on the clock the run
        // is bounded against is a read of that clock, and the clock belongs to
        // the host the run is about to be handed (ADR-0006).
        bound: match (seconds, asked_to_wedge(&args)) {
            (None, _) => Bound::Whenever,
            (Some(after), true) => Bound::Wedges(after),
            (Some(after), false) => Bound::Stops(after),
        },
    };
    ...
    let mut region = match favjit_host_macos::trace_descriptor(favjit_engine::supervision::TRACE) {
        ...
        Some(fd) => {
            match favjit_host_macos::Region::from_fd(fd, favjit_engine::supervision::TRACE_BYTES) {
                ...
                Err(error) => {
                    // The sentence composed here and not in the host, because
                    // what a machine's own account reads like inside one is not
                    // the machine's to say (`favjit_host::Trouble`).
                    error!("no trace, so nothing to read a failure off: the region on fd {fd}: {error}");
                    None
                }
            }
        }
...
/// Whichever end of the watchdog link this process was given, one read each and
/// each then stopped from blocking.
///
/// Sequenced here rather than inside the host: reading the name the descriptor
/// came under and telling the machine not to block on it are two calls, and a
/// host makes one of those per operation (ADR-0006) — so the order is this
/// program's and the four calls are asked for one at a time.
///
...
/// costs is the protection, which is why this program says so.
fn supervisor() -> Supervisor {
    Supervisor {
        probe: do_not_block(supervisor_descriptor(PROBE)),
        heartbeat: do_not_block(supervisor_descriptor(HEARTBEAT)),
    }
}
...
fn replay(path: &Path, from: Option<&str>) -> i32 {
    ...
    for from in checkpoints {
        let mut host = DryRun::new(PathBuf::new(), supervisor());
        println!("\nfrom checkpoint {from}:");
        let writing = host.instead_of_the_output();
        let wrote = match sink::replay(Layout::dudrack(), None, &trace, from, &mut host, writing) {
            Replayed::Events { events, wrote } => {
                info!("{events} event(s) replayed");
                wrote
            }
...
fn scan_usages(seconds: f64, control: PathBuf) -> i32 {
    ...
    let mut host = MacOsHost::new((None, None), control, supervisor());
    let watched = sink::watch(&mut host, Bound::Stops(Duration::from_secs_f64(seconds)));

    // Printed at the end rather than as they arrive, because a usage repeats
    // every time the key is pressed and this list is what the mode is for.
    println!("\n{} unnamed usage(s):", watched.unnamed.len());
    for (device, page, usage) in &watched.unnamed {
        println!(
            "  device {} page {page:#06x} usage {usage:#04x} ({usage})",
            device.0
        );
    }
    stopped(watched.ending)
}
