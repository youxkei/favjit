...
use favjit_host::source::{
    Answering, Answers, Arrived, Driving, Opening, SourceHost, Stroke, Suppressing,
};
...
pub enum Did {
    ...
    Warned,
    /// Asked to stop this thread for good.
    ///
    /// Noted and answered, where a real machine would never answer: what a case
    /// about a run that hangs on purpose asserts is that it asked, and a script
    /// that hung here would be a suite that stopped.
    Parked,
}
...
pub enum OutputCall {
    ...
    AskedForTheKeyboard,
    AskedForThePointer,
    /// One whole frame the run put on the connection, as it wrote it.
    ///
    /// The bytes and not which frame they are, because that is what crosses:
    /// a script holding the kind would be one asserting the run's own idea of
    /// what it sent rather than what the service would have read.
    Frame(Vec<u8>),
}
...
/// stopping.
///
/// The silence is the strictest the service is ever configured for and not the
/// margin a particular installed version happens to allow: one that tracked the
/// version on the machine would stop asking whether a run beats in time the
/// moment a version with more room arrived, and what a run may depend on is the
/// smaller of the two.
const SERVICE_BEATS_EVERY: Duration = Duration::from_secs(3);
const SERVICE_HEARS_NOTHING_FOR: Duration = Duration::from_secs(5);
...
pub struct SimOutput {
    ...
    tears_a_frame: bool,
    /// Whether it refuses to let a read be given a bound.
    bounds_reads: bool,
    /// What the next reads answer with before the service says anything of its
    /// own, oldest first.
    ///
    /// A queue and not one answer, because what these are about is the read
    /// after: a stream socket cutting one read short is a read worth making
    /// again, and a case that could script only the interruption would never
    /// find out whether the run made it.
    reads_first: VecDeque<favjit_host::output::Took>,
...
impl favjit_host::output::OutputHost for SimOutput {
    ...
    fn read_some(&mut self, into: &mut [u8]) -> favjit_host::output::Took {
        // What is already on the wire first, and at no cost in time: those bytes
        // are there, so a read of them that waited would be a service charging
        // for what it has already written.
        ...
        // Then what a case scripted about the socket itself, which is after the
        // frames rather than in front of them: those frames are what brings the
        // devices up, so a socket answer taken first would be one given to a run
        // that has not finished connecting — and a case about the serving loop
        // would never reach it.
        if let Some(took) = self.reads_first.pop_front() {
            return took;
        }
    ...
    /// Read for which frame it is out of the type byte the framing puts behind
    /// the length, the way the real service reads one: a script that was told
    /// which frame this was would be a service taking the run's word for it.
    fn put_a_frame(&mut self, bytes: &[u8]) -> i32 {
        let kind = bytes.get(LENGTH).copied();
        ...
        match (kind, self.refuses_beats) {
            (Some(HEARTBEAT), Some(code)) => {
                self.note(OutputCall::Frame(bytes.to_vec()));
                code
            }
            _ => {
                self.heard(OutputCall::Frame(bytes.to_vec()));
                0
            }
        }
    }
...
/// One keyboard this platform has, as the sequence that makes it readable.
///
/// Named by the registry's own number rather than by the number the run gives it,
/// because that is the whole of what the loop's own `Known` is for: two findings
/// of the same keyboard carry the same name and different findings, and the run
/// keeping it once is what a case about that asserts.
#[derive(Debug, Clone)]
pub struct SimKeyboard {
    named: u64,
    /// The properties the machine describes it with, in the shape IOKit gives
    /// them.
    ///
    /// Raw and not the run's reading of them, because the reading is the run's:
    /// which of these make a keyboard, and which say it is this machine's own,
    /// are tables `engine` holds (ADR-0006) — so a script handing over the
    /// reading would assert it against itself.
    ...
    vendor_id: Option<i64>,
    product_id: Option<i64>,
    /// How many elements it declares, and which of them will not go on a queue.
    elements: usize,
    /// What it reports once it is delivering, in order.
    values: VecDeque<(u32, u32, i64)>,
    /// Whether it turns up after the run started rather than being here already.
    arrives_later: bool,
    /// What a seize of it answers, where that is not nothing.
    refuses_seizing: Option<i32>,
    /// Whether the machine will name it at all.
    nameable: bool,
    /// Whether opening the finding produces a device.
    openable: bool,
    /// Whether it goes away while the run is reading it.
    goes: bool,
}

impl SimKeyboard {
    /// One external keyboard that is here, declares one element and reports
    /// nothing.
    ///
    /// A keyboard by the two usages that name the class, since anything else is a
    /// device the run declines: a default that was not one would make every case
    /// below say so before it could say anything of its own.
    pub fn named(named: u64) -> Self {
        Self {
            named,
            primary_usage_page: Some(0x01),
            primary_usage: Some(0x06),
            transport: Some(String::from("USB")),
            product: Some(String::from("TrackPoint Keyboard II")),
            vendor_id: Some(0x17ef),
            product_id: Some(0x60e1),
            elements: 1,
            values: VecDeque::new(),
            arrives_later: false,
            refuses_seizing: None,
            nameable: true,
            openable: true,
            goes: false,
        }
    ...
    /// The Mac's own, which it says by the bus it sits on.
    pub fn built_in(mut self) -> Self {
        self.transport = Some(String::from("FIFO"));
        self.product = Some(String::from("Apple Internal Keyboard / Trackpad"));
        self
    }

    /// Something the machine describes that is no keyboard at all.
    pub fn not_a_keyboard(mut self) -> Self {
        self.primary_usage = Some(0x02);
        self
    }

    pub fn reporting(mut self, page: u32, usage: u32, value: i64) -> Self {
        self.values.push_back((page, usage, value));
        self
    }

    pub fn arriving_later(mut self) -> Self {
        self.arrives_later = true;
        self
    }

    pub fn that_will_not_be_seized(mut self, code: i32) -> Self {
        self.refuses_seizing = Some(code);
        self
    }

    pub fn the_machine_will_not_name(mut self) -> Self {
        self.nameable = false;
        self
    }

    pub fn that_will_not_open(mut self) -> Self {
        self.openable = false;
        self
    }

    pub fn that_goes_away(mut self) -> Self {
        self.goes = true;
        self
    }
}

/// Which kind of thing a handle this platform handed over is.
///
/// A tag in the number rather than four lists to look a bare index up in,
/// because the run hands each of these back and nothing else: a platform that
/// could not tell a queue's handle from a device's would answer a call about one
/// with the other and look right doing it.
const A_FINDING: u64 = 1_000;
const A_DEVICE: u64 = 2_000;
const ITS_ELEMENTS: u64 = 3_000;
const ITS_QUEUE: u64 = 4_000;

/// Which keyboard a handle of that kind names.
fn which(handle: favjit_host::Handed) -> usize {
    (handle.0 % 1_000) as usize
}
...
struct SimCapture {
    ...
    hooked: bool,
    /// The keyboards this platform has, where a case asked for any.
    ///
    /// Empty for every case that does not, which is what keeps the two ways a
    /// keyboard can reach a run apart: a script that says what arrives puts it on
    /// the stream under the number it gave, and the numbers in every assertion
    /// about a device are that script's. A case about the sequence that makes one
    /// readable asks for this instead, and then the numbers are the loop's.
    keyboards: Vec<SimKeyboard>,
    /// Which of them the listing has handed over, and which the arrivals have.
    listed: usize,
    arrived: usize,
    /// The queues that have said they have values, oldest first.
    with_values: VecDeque<usize>,
    /// The ones that have gone and not yet been given back.
    gone: VecDeque<usize>,
    /// Which of them the run holds a queue over, so a device going can be asked
    /// for its queue the way IOKit is asked.
    reading: Vec<usize>,
    /// Every call the sequence made, in order.
    asked: Arc<Mutex<Vec<CaptureCall>>>,
    /// The token the run hands turns over with, where a case asked for the two
    /// loops alongside each other.
    turning: Option<Turning>,
}

/// One operation the sequence that makes a keyboard readable is built of.
///
/// Only the ones an order is asserted on: what a case about this sequence is for
/// is which call comes before which, and a list carrying every one of the forty
/// would be a case that fails whenever a call is added rather than when an order
/// changes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CaptureCall {
    OpenedAPort,
    AskedToBeToldAboutDevices,
    PutThatPortOnThisLoop,
    LookedAtWhatIsHere,
    FoundOneHere { named: u64 },
    FoundOneArriving { named: u64 },
    Opened { named: u64 },
    WatchedForItsRemoval { named: u64 },
    Seized { named: u64, exclusive: bool },
    MadeAQueueOver { named: u64 },
    PutAnElementOn { named: u64, at: usize },
    WatchedForItsValues { named: u64 },
    StartedItDelivering { named: u64 },
    LetGoOfTheFinding { named: u64 },
    GaveItBack { named: u64 },
}

impl SimCapture {
    fn note(&self, call: CaptureCall) {
        self.asked.lock().unwrap().push(call);
    }

    /// The registry's own name for the keyboard a handle names, and a name for
    /// nothing where the handle names no keyboard this platform has.
    fn named(&self, handle: favjit_host::Handed) -> u64 {
        self.keyboards
            .get(which(handle))
            .map_or(u64::MAX, |keyboard| keyboard.named)
    }
}

/// What the capture loop and the run pass between them while the two are turned
/// alongside each other.
///
/// A token the run holds rather than two loops racing, which is what keeps a
/// case readable: the real platform turns this loop on a thread of its own, so a
/// suite that turned it inline could not drive a seize the run asks for *after*
/// being told there is a keyboard — and one that let it run free would put the
/// two in an order no case could state. The loop blocks in one place only, so
/// handing it one turn at a time is the whole of the arrangement.
#[derive(Default)]
struct Alongside {
    /// Turns the run has let the loop take, and whether it will let it take more.
    allowed: usize,
    closed: bool,
    /// Whether the loop is back in its wait, which is what says a turn finished.
    waiting: bool,
    /// What the loop delivered, for the run to read onto its own stream.
    delivered: VecDeque<EventKind>,
    /// What the run asked of the loop, oldest first, and what those answered.
    asks: VecDeque<favjit_host::capture::Ask>,
    answers: Vec<i32>,
}

/// The two ends of that arrangement, and the way each wakes the other.
type Turning = Arc<(Mutex<Alongside>, std::sync::Condvar)>;

/// Let the loop take one whole turn and come back to its wait.
///
/// Waited out rather than fired and forgotten, because what a case asserts is an
/// order: a run that handed over a turn and carried on would be one whose calls
/// and the loop's interleave differently from one run to the next.
fn one_turn(turning: &Turning) {
    let (lock, woken) = &**turning;
    ...
    state.allowed += 1;
    woken.notify_all();
    while state.allowed > 0 && !state.closed {
        state = woken.wait(state).unwrap();
    }
}

impl favjit_host::capture::Reading for SimCapture {
    fn turn_the_loop(&mut self, _poll: Duration) {
        let Some(turning) = self.turning.clone() else {
        ...
        state.allowed = state.allowed.saturating_sub(1);
        state.waiting = false;
        woken.notify_all();
    }
    ...
    /// Put on the run's stream where a case asked for the two alongside each
    /// other, and dropped otherwise: a script that says what arrives has already
    /// put it there, so one delivered here as well would be the same keyboard
    /// twice.
    fn deliver(&mut self, event: EventKind) -> bool {
        let Some(turning) = self.turning.clone() else {
            return true;
        };
        turning.0.lock().unwrap().delivered.push_back(event);
        true
    }

    fn over(&mut self) -> bool {
        match self.turning.as_ref() {
            None => self.turns == 0,
            Some(turning) => turning.0.lock().unwrap().closed,
        }
    }
}
...
impl favjit_host::capture::SourceCapture for SimCapture {
    ...
    fn what_is_waiting(&mut self) -> Option<favjit_host::capture::Waiting> {
        None
    }

    fn take_what_is_waiting(&mut self) -> Option<EventKind> {
        None
    }
}
...
impl favjit_host::capture::SinkCapture for SimCapture {
    fn open_a_notification_port(&mut self) -> bool {
        self.note(CaptureCall::OpenedAPort);
        true
    }

    fn ask_to_be_told_about_devices(&mut self) -> bool {
        self.note(CaptureCall::AskedToBeToldAboutDevices);
        true
    }

    fn put_that_port_on_this_loop(&mut self) {
        self.note(CaptureCall::PutThatPortOnThisLoop);
    }

    fn look_at_the_devices_here(&mut self) -> bool {
        self.note(CaptureCall::LookedAtWhatIsHere);
        true
    }

    /// The next keyboard a case said was already here, as a finding.
    ///
    /// A finding and not the device, because those are two things on a real
    /// platform: the registry hands over an entry and opening it is what produces
    /// something that can be seized — so a platform that answered with the device
    /// would be one where the sequence between them could not be driven.
    fn next_device_here(&mut self) -> Option<favjit_host::Handed> {
        let at = self.listed;
        let (at, named) = self
            .keyboards
            .iter()
            .enumerate()
            .skip(at)
            .find(|(_, keyboard)| !keyboard.arrives_later)
            .map(|(at, keyboard)| (at, keyboard.named))?;
        self.listed = at + 1;
        self.note(CaptureCall::FoundOneHere { named });
        Some(favjit_host::Handed(A_FINDING + at as u64))
    }

    fn next_arriving_device(&mut self) -> Option<favjit_host::Handed> {
        let at = self.arrived;
        let (at, named) = self
            .keyboards
            .iter()
            ...
            .map(|(at, keyboard)| (at, keyboard.named))?;
        self.arrived = at + 1;
        self.note(CaptureCall::FoundOneArriving { named });
        Some(favjit_host::Handed(A_FINDING + at as u64))
    }

    fn name_of(&mut self, found: favjit_host::Handed) -> Option<u64> {
        let keyboard = self.keyboards.get(which(found))?;
        keyboard.nameable.then_some(keyboard.named)
    }

    fn open_it(&mut self, found: favjit_host::Handed) -> Option<favjit_host::Handed> {
        let at = which(found);
        let keyboard = self.keyboards.get(at)?;
        if !keyboard.openable {
            return None;
        }
        let named = keyboard.named;
        self.note(CaptureCall::Opened { named });
        Some(favjit_host::Handed(A_DEVICE + at as u64))
    }

    fn let_go_of_the_finding(&mut self, found: favjit_host::Handed) {
        let named = self.named(found);
        self.note(CaptureCall::LetGoOfTheFinding { named });
    }

    fn watch_for_its_removal(&mut self, device: favjit_host::Handed) {
        let named = self.named(device);
        self.note(CaptureCall::WatchedForItsRemoval { named });
    }

    fn put_it_where_reports_arrive(&mut self, _device: favjit_host::Handed) {}

    /// What the machine says about it, under the number the run just gave it.
    ///
    /// Built here from what the script said about the keyboard rather than
    /// carrying a whole event: the number is the run's, so a script that handed
    /// one over would be asserting the numbering against itself.
    fn describe(&mut self, device: favjit_host::Handed, as_this: DeviceId) -> EventKind {
        match self.keyboards.get(which(device)) {
            None => EventKind::DeviceLost(as_this),
            Some(keyboard) => EventKind::HidDeviceFound {
                device: as_this,
                primary_usage_page: keyboard.primary_usage_page,
                primary_usage: keyboard.primary_usage,
                transport: keyboard.transport.clone(),
                product: keyboard.product.clone(),
    ...
    fn next_ask(&mut self) -> Option<favjit_host::capture::Ask> {
        *self.answered.lock().unwrap() += 1;
        ...
        let ask = turning.0.lock().unwrap().asks.pop_front();
        ask
    }

    fn seize(&mut self, device: favjit_host::Handed, exclusive: bool) -> i32 {
        let Some(keyboard) = self.keyboards.get(which(device)) else {
            return -1;
        };
        let named = keyboard.named;
        let refused = keyboard.refuses_seizing;
        self.note(CaptureCall::Seized { named, exclusive });
        refused.unwrap_or(0)
    }
    ...
    fn every_element_of(&mut self, device: favjit_host::Handed) -> Option<favjit_host::Handed> {
        let at = which(device);
        self.keyboards
            .get(at)
            .map(|_| favjit_host::Handed(ITS_ELEMENTS + at as u64))
    }

    fn make_a_queue_over(&mut self, device: favjit_host::Handed) -> Option<favjit_host::Handed> {
        let at = which(device);
        let named = self.keyboards.get(at)?.named;
        self.note(CaptureCall::MadeAQueueOver { named });
        Some(favjit_host::Handed(ITS_QUEUE + at as u64))
    }

    fn how_many_elements_of(&mut self, elements: favjit_host::Handed) -> usize {
        self.keyboards
            .get(which(elements))
            .map_or(0, |keyboard| keyboard.elements)
    }

    fn has_an_element_at(&mut self, elements: favjit_host::Handed, at: usize) -> bool {
        self.keyboards
            .get(which(elements))
            .is_some_and(|keyboard| at < keyboard.elements)
    }

    fn put_one_element_on(
        &mut self,
        _queue: favjit_host::Handed,
        elements: favjit_host::Handed,
        at: usize,
    ) -> bool {
        let named = self.named(elements);
        self.note(CaptureCall::PutAnElementOn { named, at });
        true
    }

    fn let_go_of_the_elements(&mut self, _elements: favjit_host::Handed) {}

    fn watch_for_its_values(&mut self, queue: favjit_host::Handed) {
        let named = self.named(queue);
        self.note(CaptureCall::WatchedForItsValues { named });
    }

    fn put_it_where_values_arrive(&mut self, _queue: favjit_host::Handed) {}

    /// Where what the keyboard reports goes on the queue, because that is where a
    /// running one would take it: a value waiting before the start would be one
    /// delivered by a queue nothing had started.
    fn start_it_delivering(&mut self, queue: favjit_host::Handed) {
        let at = which(queue);
        let Some(keyboard) = self.keyboards.get(at) else {
            return;
        };
        let named = keyboard.named;
        let has_values = !keyboard.values.is_empty();
        let goes = keyboard.goes;
        self.note(CaptureCall::StartedItDelivering { named });
        self.reading.push(at);
        let _ = has_values.then(|| self.with_values.push_back(at));
        let _ = goes.then(|| self.gone.push_back(at));
    }
    ...
    fn answer_the_ask(&mut self, code: i32) {
        ...
        let (lock, woken) = &*turning;
        lock.lock().unwrap().answers.push(code);
        woken.notify_all();
    }

    fn next_thing_with_values(&mut self) -> Option<favjit_host::Handed> {
        let at = self.with_values.pop_front()?;
        Some(favjit_host::Handed(ITS_QUEUE + at as u64))
    }

    /// One element value off that queue, under this platform's own place for the
    /// keyboard it came from.
    ///
    /// The place and not a number a script chose, for the reason `describe`'s
    /// number is the run's: a value carrying a scripted number would be a case
    /// asserting its own numbering.
    fn next_value_of(&mut self, from: favjit_host::Handed) -> Option<EventKind> {
        let at = which(from);
        let (page, usage, value) = self.keyboards.get_mut(at)?.values.pop_front()?;
        Some(EventKind::HidValue {
            device: DeviceId(at as u64),
            page,
            usage,
            stamp: 1,
            value,
    ...
    fn nothing_more_of(&mut self, from: favjit_host::Handed) -> EventKind {
        EventKind::HidValuesDone {
            device: DeviceId(which(from) as u64),
        }
    }
    ...
    fn next_gone(&mut self) -> Option<favjit_host::Handed> {
        let at = self.gone.pop_front()?;
        Some(favjit_host::Handed(A_DEVICE + at as u64))
    }

    fn its_queue(&mut self, device: favjit_host::Handed) -> Option<favjit_host::Handed> {
        let at = which(device);
        self.reading
            .contains(&at)
            .then_some(favjit_host::Handed(ITS_QUEUE + at as u64))
    }
    ...
    fn give_it_back(&mut self, device: favjit_host::Handed) -> i32 {
        let named = self.named(device);
        self.note(CaptureCall::GaveItBack { named });
        0
    }

    fn forget_it(&mut self, device: favjit_host::Handed) -> Option<DeviceId> {
        Some(DeviceId(which(device) as u64))
    }
}

/// A machine's list of attached devices, as the enumeration `--devices` reads.
///
/// The count, the room and the path each answered separately, because that is the
/// shape the run drives: a double that handed the whole list over would be a
/// script for an enumeration nobody enumerates.
#[derive(Default)]
pub struct SimListing {
    /// What each place holds, and the path it will answer with.
    ///
    /// `None` for a kind the run has no name for, which is a place to pass over
    /// rather than the end of the listing.
    devices: Vec<(Option<favjit_host::source::Kind>, String)>,
    /// How many of them the machine will actually look at, where that is fewer
    /// than it said it had.
    looks_at: Option<usize>,
    /// The places whose path will not fit in the room the machine asked for.
    without_a_path: Vec<usize>,
    /// Every call the run made, in order.
    calls: Vec<ListingCall>,
}

/// One operation the enumeration is made of.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ListingCall {
    AskedHowMany,
    Looked { how_many: usize },
    AskedWhatIsAt { place: usize },
    AskedHowLongThePathIs { place: usize },
    AskedForThePath { place: usize, room: usize },
}
...
impl SimListing {
    ...
    pub fn with_a_keyboard_at(mut self, path: &str) -> Self {
        self.devices.push((
            Some(favjit_host::source::Kind::Keyboard),
            String::from(path),
        ));
        self
    }

    pub fn with_a_pointer_at(mut self, path: &str) -> Self {
        self.devices
            .push((Some(favjit_host::source::Kind::Pointer), String::from(path)));
        self
    }

    /// One the machine lists and the run has no name for.
    pub fn with_something_it_will_not_name(mut self) -> Self {
        self.devices.push((None, String::new()));
        self
    }

    /// A machine that says it has some and then looks at fewer.
    pub fn that_looks_at(mut self, how_many: usize) -> Self {
        self.looks_at = Some(how_many);
        self
    }

    /// A machine whose path for this place will not go in the room it asked for.
    pub fn whose_path_will_not_fit(mut self, place: usize) -> Self {
        self.without_a_path.push(place);
        self
    }

    pub fn calls(&self) -> &[ListingCall] {
        &self.calls
    }
}

impl favjit_host::source::Listing for SimListing {
    fn how_many(&mut self) -> usize {
        self.calls.push(ListingCall::AskedHowMany);
        self.devices.len()
    }

    fn look(&mut self, how_many: usize) -> usize {
        self.calls.push(ListingCall::Looked { how_many });
        self.looks_at.unwrap_or(how_many)
    }

    fn at(&mut self, place: usize) -> Option<favjit_host::source::Kind> {
        self.calls.push(ListingCall::AskedWhatIsAt { place });
        self.devices.get(place).and_then(|(kind, _)| *kind)
    }

    fn how_long_its_path_is(&mut self, place: usize) -> usize {
        self.calls
            .push(ListingCall::AskedHowLongThePathIs { place });
        self.devices.get(place).map_or(0, |(_, path)| path.len())
    }

    fn its_path(&mut self, place: usize, room: usize) -> Option<String> {
        self.calls
            .push(ListingCall::AskedForThePath { place, room });
        if self.without_a_path.contains(&place) {
            return None;
        }
        self.devices.get(place).map(|(_, path)| path.clone())
    }
}
...
struct Machine {
    ...
    output_bounds_reads: bool,
    output_reads_first: VecDeque<favjit_host::output::Took>,
    /// The keyboards the capture loop is to find, where a case asked for any.
    capture_keyboards: Vec<SimKeyboard>,
    /// What that loop asked of the platform, shared because the loop is turned
    /// somewhere this object has been given away to.
    capture_asked: Arc<Mutex<Vec<CaptureCall>>>,
    /// The token the two loops pass turns with, and nothing where a case did not
    /// ask for them alongside each other.
    turning: Option<Turning>,
    /// The thread the capture loop is turned on, kept so that it is told to stop
    /// and waited for: a loop still blocked on its token when a case ends is a
    /// thread the next case inherits.
    turned_alongside: Option<std::thread::JoinHandle<()>>,
    ...
    warnings: Vec<String>,
    /// And the lines it put where a person reads them, kept apart for the reason
    /// the two calls are apart: one is a problem and the other is the answer.
    said: Vec<String>,
...
/// Tell the loop turned alongside this one to stop, and wait for it.
///
/// Waited for rather than left: the loop blocks on its token, so one still
/// blocked when a case ends is a thread the next case inherits — and a suite
/// whose cases share threads is one whose order is not the case's.
impl Drop for Machine {
    fn drop(&mut self) {
        let Some(turning) = self.turning.take() else {
            return;
        };
        let (lock, woken) = &*turning;
        lock.lock().unwrap().closed = true;
        woken.notify_all();
        let _ = self.turned_alongside.take().map(|thread| thread.join());
    }
}
...
impl Default for Machine {
    ...
    fn default() -> Self {
        Self {
            ...
            output_bounds_reads: true,
            output_reads_first: VecDeque::new(),
            capture_keyboards: Vec::new(),
            capture_asked: Arc::new(Mutex::new(Vec::new())),
            turning: None,
            turned_alongside: None,
...
impl SimHost {
    ...
    pub fn whose_output_asks(self, frame: Vec<u8>) -> Self {
        self.scripted(|machine| machine.whose_output_asks(frame));
        self
    }

    pub fn whose_output_will_not_bound_its_reads(self) -> Self {
        self.scripted(Machine::whose_output_will_not_bound_its_reads);
        self
    }

    pub fn whose_output_socket_answers(self, took: favjit_host::output::Took) -> Self {
        self.scripted(|machine| machine.whose_output_socket_answers(took));
        self
    }

    pub fn whose_beats_reach_nothing(self) -> Self {
        self.scripted(Machine::whose_beats_reach_nothing);
        self
    }

    pub fn whose_capture_loop_finds(self, keyboards: Vec<SimKeyboard>) -> Self {
        self.scripted(|machine| machine.whose_capture_loop_finds(keyboards));
        self
    }

    pub fn capture_asked(&self) -> Vec<CaptureCall> {
        self.0.borrow().capture_asked()
    }
...
impl Machine {
    ...
    /// A machine whose output device comes up and then asks this of the run.
    ///
    /// After the frame that brings the devices up rather than instead of it,
    /// because that frame is what a run waits for: a question asked in front of
    /// it would be one asked of a run that has not finished connecting.
    ///
    /// Scripted as what the service sends and never as the answer, for the reason
    /// the readiness flags are: which frame is a question and what goes back for
    /// one is the run's own reading (ADR-0006), so a script carrying the answer
    /// would assert that reading against itself.
    pub fn whose_output_asks(mut self, frame: Vec<u8>) -> Self {
    ...
    /// A machine that will not let a read on that connection be given a bound.
    ///
    /// Its own case rather than one of the readiness flags: a bound that will not
    /// go on is a read with no way back out, so what the run has to do about it is
    /// let the connection go rather than wait in it — and a wait with no bound is
    /// the one failure no keystroke ever reaches.
    pub fn whose_output_will_not_bound_its_reads(mut self) -> Self {
        self.output_bounds_reads = false;
        self
    }

    /// A machine with a watchdog behind it that the beats do not reach.
    ///
    /// Not the same as having no watchdog: a run nothing is supervising has no
    /// pipe to write down and nothing to say about it, while one whose beats go
    /// nowhere is about to be ended for a broken link rather than for a fault —
    /// which is the one thing worth saying before it happens (ADR-0008).
    pub fn whose_beats_reach_nothing(mut self) -> Self {
        self.beats_arrive = false;
        self
    }

    /// A machine whose keyboards the capture loop has to find and make readable.
    ///
    /// The other way round from every case that scripts a keystroke: there the
    /// numbers are the script's and the loop finds nothing, because what is being
    /// asserted is what a keystroke turns into. Here the loop is what is being
    /// asserted, so the numbers are its own — which is why the two are separate
    /// ways of saying a machine has a keyboard rather than one that tries to be
    /// both.
    pub fn whose_capture_loop_finds(mut self, keyboards: Vec<SimKeyboard>) -> Self {
        self.capture_keyboards = keyboards;
        self.turning = Some(Turning::default());
        self
    }

    /// What that loop asked of the platform, in order.
    pub fn capture_asked(&self) -> Vec<CaptureCall> {
        self.capture_asked.lock().unwrap().clone()
    }

    /// A machine whose socket answers this way before the service says anything.
    ///
    /// The socket's own answers and not the service's, which is why they are
    /// scripted apart from the frames: how a stream socket ends a read — with
    /// nothing, with fewer bytes than were asked for, cut short, or with no bytes
    /// at all — is the machine underneath the protocol, and what the run makes of
    /// each is its own (ADR-0006).
    pub fn whose_output_socket_answers(mut self, took: favjit_host::output::Took) -> Self {
        self.output_reads_first.push_back(took);
        self
    }
    ...
    /// And what it showed the person, in order.
...
impl Machine {
    ...
    ///
    /// Unless a case asked for keyboards through this loop, which is the opposite
    /// trade and why it is opt-in: the sequence that makes one readable is then
    /// what is being driven, and the numbers are the loop's because that is the
    /// thing under test.
    fn look_for_devices(&mut self, work: favjit_host::capture::SinkLoop) -> bool {
        ...
        let mut capture = SimCapture {
            ...
            hooked: false,
            keyboards: self.capture_keyboards.clone(),
            listed: 0,
            arrived: 0,
            with_values: VecDeque::new(),
            gone: VecDeque::new(),
            reading: Vec::new(),
            asked: Arc::clone(&self.capture_asked),
            turning: self.turning.clone(),
        };
        // Alongside where a case asked for keyboards through this loop, and to a
        // standstill otherwise: the run only reaches what it asks of this loop
        // after it has been told there is a keyboard, so a loop that finished
        // before the converting one began is one no seize could reach.
        match self.turning.clone() {
            None => work(&mut capture),
            Some(turning) => {
                self.turned_alongside = Some(std::thread::spawn(move || work(&mut capture)));
                // One turn before returning, so the keyboards already attached
                // are on the stream by the time the converting loop reads it —
                // which is the order a real platform's listing arrives in.
                one_turn(&turning);
            }
        }
        true
    ...
    fn take_device(&mut self, device: DeviceId, exclusive: bool) -> i32 {
        ...
        // Through the loop where one is turned alongside this one, because that
        // is where the seize is made: a code answered here instead would be the
        // script's word for what the platform said.
        if let Some(code) = self.ask_alongside(favjit_host::capture::Ask::Take {
            device: favjit_host::Handed(device.0),
            exclusive,
        }) {
            let _ = (code == 0 && exclusive).then(|| self.held.push(device));
            return code;
        }
        let code = self
            .refuses_to_take
...
impl Machine {
    /// Let the loop turned alongside this one take one turn, and put what it
    /// delivered on the stream.
    ///
    /// Nothing at all where no case asked for the two alongside each other,
    /// which is what keeps every other case as it was: a script that says what
    /// arrives has already put it on the stream, so a turn taken here as well
    /// would be the same keyboard twice.
    fn turn_alongside(&mut self) {
        let Some(turning) = self.turning.clone() else {
            return;
        };
        one_turn(&turning);
        let delivered: Vec<EventKind> = turning.0.lock().unwrap().delivered.drain(..).collect();
        let at = self.now;
        self.inbound
            .extend(delivered.into_iter().map(|kind| HostEvent { at, kind }));
    }

    /// Put one ask where that loop reads it, and turn it until it has answered.
    ///
    /// Turned to the answer rather than left for the next turn, because the run
    /// reads what a seize said as the call's own answer: a machine that answered
    /// before the loop had made the call would be one whose answer was the
    /// script's rather than the platform's.
    fn ask_alongside(&mut self, ask: favjit_host::capture::Ask) -> Option<i32> {
        let turning = self.turning.clone()?;
        let answered = {
            let (lock, woken) = &*turning;
            let mut state = lock.lock().unwrap();
            state.asks.push_back(ask);
            woken.notify_all();
            state.answers.len()
        };
        // Bounded, so that a loop which will never make the call leaves a case
        // failing on what it asserts rather than hanging: one turn per ask it
        // could be behind on, and one over.
        for _ in 0..4 {
            one_turn(&turning);
            let state = turning.0.lock().unwrap();
            if state.answers.len() > answered {
                return state.answers.last().copied();
            }
        }
        None
    }
    ...
    fn read_device(&mut self, device: DeviceId) -> bool {
        ...
        // Through that loop for the reason the seize is, and read as whether it
        // worked because that is what this call answers: the queue and the
        // elements behind it are several calls, and which of them failed is
        // already in the recording.
        if let Some(code) = self.ask_alongside(favjit_host::capture::Ask::Read {
            device: favjit_host::Handed(device.0),
        }) {
            let _ = (code == 0).then(|| self.reading.push(device));
            return code == 0;
        }
...
impl SinkInputHost for SimHost {
    ...
    /// Noted rather than acted on: this machine's clock is the script's own, so
    /// there is no origin for a run to move — and what a test asserts about a
    /// timestamp is what the script said, not when it asked.
    fn start_a_clock(&mut self) {
        self.0.borrow_mut().note(Did::StartedAClock);
    }
}
...
impl Host for SimHost {
    ...
    fn say(&mut self, line: core::fmt::Arguments) {
        self.0.borrow_mut().say(line);
    }

    fn park(&mut self) {
        self.0.borrow_mut().park();
    }
...
impl Host for Machine {
    ...
    fn next_event(&mut self, deadline: Instant) -> Option<HostEvent> {
        // One turn of the loop turned alongside this one before the wait, which
        // is what a real platform's would have taken while the run was busy: a
        // run that waited without letting it turn would be one whose keyboards
        // arrive only when something else already has.
        self.turn_alongside();
    ...
    /// Kept apart from the warnings, for the reason the call is: a case about
    /// what a run shows a person would otherwise pass on a warning that happened
    /// to contain the word.
    fn say(&mut self, line: core::fmt::Arguments) {
...
impl Machine {
    fn reach_the_output(
        ...
        Ok(Box::new(SimOutput {
            // Every connection gets them, rather than the first one taking
            // them off the script: the service builds its virtual devices
            // afresh for whoever connects and reports them ready to that
            // client, so a second connection told nothing about itself would
            // be a scripted service no real one behaves like.
            ...
            bounds_reads: self.output_bounds_reads,
            reads_first: self.output_reads_first.clone(),
            tore_one: false,
...
impl SimSource {
    ...
    pub fn shown_here(&self) -> Vec<Vec<Stroke>> {
        self.0.borrow().shown_here()
    }
...
impl SourceHost for SimSource {
    ...
    /// timestamp is what the script said, not when it asked.
    fn start_a_clock(&mut self) {
        self.0.borrow_mut().started_a_clock();
    }
    ...
    fn show_here(&mut self, strokes: &[Stroke]) {
        self.0.borrow_mut().show_here(strokes);
    }
}
...
struct Source {
    ...
    /// What the run showed this machine, one entry per call and the strokes of
    /// each in the order they were handed over.
    ///
    /// The strokes and not keys on the stream: what these stand in for are
    /// strokes the procedures are given and drop, so a script that delivered
    /// them would be asserting that this machine reads its own.
    shown_here: Vec<Vec<Stroke>>,
...
impl Source {
    ...
    fn started_a_clock(&mut self) {
        let _ = self.source_call(SourceCall::StartedAClock);
    }
...
impl DiscoveryHost for SimSource {
    ...
    fn a_sink_at(&mut self, address: [u8; 4], port: u16) -> Option<favjit_host::Sink> {
        self.0
            .borrow_mut()
            .resolve_discovered("", port, Some(address))
    }

    fn resolve_discovered(&mut self, host: &str, port: u16) -> Option<favjit_host::Sink> {
        self.0.borrow_mut().resolve_discovered(host, port, None)
    }
}
...
pub struct Answered {
    ...
    /// What the answer showed this machine, one entry per call and the strokes
    /// of each in the order they were handed over.
    ///
    /// The strokes and not a count, because the order is the behaviour: what
    /// stops a modifier reading alone is a key between its two halves, and a
    /// count could not be asked whether one was there.
    pub shown: Vec<Vec<Stroke>>,
...
impl Answers for Asked {
    ...
    /// Put down rather than turned into keys on the stream: what these stand in
    /// for are strokes the real procedures are given and drop, so a script that
    /// delivered them would be asserting that this machine reads its own.
    fn show_here(&self, strokes: &[Stroke]) {
        self.answered.borrow_mut().shown.push(strokes.to_vec());
    }
...
impl Source {
    ...
    fn take_input(
        ...
        if self.reads_input {
            let mut capture = SimCapture {
                ...
                hooked: false,
                // Nothing, because this is the forwarding machine's loop: what a
                // keyboard is here is a position and a make code, and the
                // sequence below is the converting machine's IOKit.
                keyboards: Vec::new(),
                listed: 0,
                arrived: 0,
                with_values: VecDeque::new(),
    ...
    fn show_here(&mut self, strokes: &[Stroke]) {
        self.shown_here.push(strokes.to_vec());
    }

    pub fn shown_here(&self) -> Vec<Vec<Stroke>> {
        self.shown_here.clone()
    }
...
impl Host for Source {
    ...
    /// Answered rather than hanging, for the reason [`Did::Parked`] is noted on
    /// the other side: nothing on this one ever asks, and a script that hung here
    /// would be a suite that stopped.
    fn park(&mut self) {}

    /// Dropped rather than kept: nothing on this side shows a person anything,
    /// and a list nothing reads would be a field to keep in step for no case.
    fn say(&mut self, _line: core::fmt::Arguments) {}
