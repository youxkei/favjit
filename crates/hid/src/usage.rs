...
pub fn named(page: u32, usage: u32) -> Option<Key> {
    ...
    if page == self::page::APPLE_VENDOR_TOP_CASE && usage == crate::KEYBOARD_FN {
        return Some(Key::Fn);
    }
...
fn on_page_seven(key: Key) -> Option<u16> {
    Some(match key {
        ...
        // key added to the vocabulary lands here too, and the table walk in this
        // module's own tests is what makes that a failing test rather than a key
        // that quietly never arrives.
        Key::Fn | _ => return None,
    })
}

/// The page and usage a keyboard would report this key's element at.
///
/// [`of`] answers the same question for every key but `Fn`, which arrives on
/// Apple's top-case page rather than the one it would be sent back out on —
/// `of` cannot name that page, since nothing here ever posts to it.
pub fn read_at(key: Key) -> Option<(u32, u16)> {
    match key {
        Key::Fn => Some((self::page::APPLE_VENDOR_TOP_CASE, crate::KEYBOARD_FN as u16)),
        _ => of(key),
    }
}
...
/// Keys a usage names on the way in that have no usage to send them back out on.
///
/// The read direction's `Fn` is the one entry: it lives on Apple's top-case page,
/// which the layout only ever matches on and never emits. `#[cfg(test)]`, unlike
/// the pub items above it, because nothing outside this module's own tests reads
/// it — the pub direction the read/write pair is checked against is a rule's
/// reachability, `engine`'s question to ask against its own layout.
#[cfg(test)]
const DROPPED: &[Key] = &[Key::Fn];

#[cfg(test)]
mod tests {
    //! What these tables can say about themselves alone, without a layout to
    //! press them against: that the two directions agree wherever both answer,
    //! that a pointer's own elements are told apart from their page's other
    //! controls, and that the read direction's gaps are exactly the ones named.
    //! Whether a *rule* can reach every key it names is `engine`'s question,
    //! asked against `Layout::dudrack` where the suite can drive it.
    ...
    fn readable() -> Vec<Key> {
        PAGES
            .iter()
            .flat_map(|&page| (0..=0xFFFFu32).filter_map(move |usage| named(page, usage)))
            .collect()
    }
    ...
    fn the_two_directions_agree_wherever_both_have_an_answer() {
        ...
        // usage that read as one key and wrote as another would convert correctly
        // and send the wrong keystroke.
        let mut disagreed = Vec::new();
        for page in PAGES {
            for usage_number in 0..=0xFFFFu32 {
                let Some(key) = named(page, usage_number) else {
                    continue;
                };
                // A key that cannot be sent at all is `DROPPED`'s to account for;
                // what this is about is the pair that can both be read and written.
    ...
    fn only_the_named_keys_are_dropped_when_passing_through() {
        // A press no rule matches is emitted as itself, so the send direction has
        // to cover the read direction and not just the keys a rule mentions.
        let mut unsendable: Vec<Key> = readable()
            .into_iter()
            .filter(|key| of(*key).is_none())
            .collect();
        let mut named_dropped = DROPPED.to_vec();
        unsendable.sort_unstable_by_key(|k| format!("{k:?}"));
        named_dropped.sort_unstable_by_key(|k| format!("{k:?}"));

        assert_eq!(
            unsendable, named_dropped,
    ...
    fn the_unmapped_list_says_what_is_actually_unmapped() {
        ...
        let wrongly_listed: Vec<Key> = UNMAPPED
            .iter()
            .copied()
    ...
    fn a_pointer_element_is_watched_and_a_neighbour_on_its_page_is_not() {
        // The generic desktop page carries the system power and sleep controls
        // beside X and Y, and admitting the page would relay those.
        assert!(pointer(0x01, POINTER_X));
        assert!(pointer(0x01, POINTER_Y));
        assert!(pointer(0x01, POINTER_WHEEL));
        assert!(pointer(0x09, 1));
        assert!(!pointer(0x01, 0x81), "system power down");
        assert!(!pointer(0x01, 0x06), "the keyboard collection");
    }
}
