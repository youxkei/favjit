...
/// The kinds of frame this run writes, out of the same numbering.
///
/// Beside the ones read above rather than in the host that writes them: what
/// makes a frame mean something is the number, and a run holding the numbers it
/// reads while a machine held the numbers it writes would be one agreement in
/// two places.
const HEARTBEAT: u8 = 0;
const HEALTH_CHECK_RESPONSE: u8 = 3;

/// One whole frame, ready to be put on the connection: its length, then its
/// type, then its body.
///
/// Big-endian for the length, which the payload behind it is not — see the note
/// on [`LENGTH`]. The type byte counts towards the length, because that is where
/// the service's own framing puts the boundary.
fn framed(message_type: u8, payload: &[u8]) -> Vec<u8> {
    let body_size = 1 + payload.len();
    let mut out = Vec::with_capacity(LENGTH + body_size);
            ...
            match answer_to(&body) {
                ...
                Answer::HealthCheck => {
                    let code = write(host, Served::HealthCheckAnswer, |host| {
                        host.put_a_frame(&framed(HEALTH_CHECK_RESPONSE, &[]))
                    });
                ...
                Answer::Acknowledge(id) => {
                    let id = id.to_vec();
                    let code = write(host, Served::Acknowledgement, |host| {
                        host.put_a_frame(&framed(RESPONSE, &id))
                    });
...
fn serving(host: &mut dyn OutputHost, beat_every: Duration) -> Lost {
    ...
    loop {
        let since = crate::clock::Clock::saturating_duration_since(host.now(), beaten);
        if since >= beat_every {
            if write(host, Served::Beat, |host| {
                host.put_a_frame(&framed(HEARTBEAT, &[]))
            }) != 0
            {
        ...
        // Bounded by what is left of this beat's interval rather than by the
        // interval itself: the bound is on one read and the frame above it is
        // however many reads it takes, so a bound of the whole interval is a
        // turn that comes back a whole interval late and a cadence of twice
        // what it was asked for. The service closes a connection it has heard
        // nothing on for a multiple of that
        // (`docs/platform/macos/virtual-hid-device.md`), so the slip is the
        // connection.
        if !host.set_read_timeout(left_of(beat_every, since)) {
...
/// What is left of an interval that much of has gone, and never nothing.
///
/// Never zero, because a read given no bound at all is a read with no bound
/// rather than one that comes straight back, and this end would then go quiet
/// for as long as the service does.
fn left_of(interval: Duration, gone: Duration) -> Duration {
    interval.saturating_sub(gone).max(THE_SHORTEST_READ)
}

/// The least a read of the connection is given.
///
/// Short enough that a beat due now is not held up by a read that has only just
/// been entered, long enough that a loop coming round on it is not a spin.
const THE_SHORTEST_READ: Duration = Duration::from_millis(20);
...
mod tests {
    ...
    #[test]
    fn a_read_is_given_what_is_left_of_the_beat_rather_than_the_whole_of_it() {
        // A read that came back with a frame partway through the interval and
        // was then followed by one given the whole interval again is a beat sent
        // at twice the cadence it was asked for. The service closes a connection
        // it has heard nothing on for less than that
        // (`docs/platform/macos/virtual-hid-device.md`), so the doubling is the
        // connection and not a late beat.
        let interval = Duration::from_secs(3);

        assert_eq!(left_of(interval, Duration::ZERO), interval);
        assert_eq!(
            left_of(interval, Duration::from_secs(1)),
            Duration::from_secs(2)
        );
        // Never nothing, however little is left or how far past the interval the
        // read is entered: a bound of zero is a read with no bound at all.
        assert_eq!(left_of(interval, interval), THE_SHORTEST_READ);
        assert_eq!(left_of(interval, Duration::from_secs(4)), THE_SHORTEST_READ);
    }
    ...
    impl OutputHost for Stream {
        ...
        fn put_a_frame(&mut self, _bytes: &[u8]) -> i32 {
            0
        }
