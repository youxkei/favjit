...
pub use favjit_host::source::{
    Answering, Answers, Arrived, At, Connected, Kind, Listing, Shown, SourceHost, Stroke,
    Suppressing,
};
...
fn answer_a_key(at: At, packed: usize, flags: i64, answers: &dyn Answers) -> isize {
    ...
    if refuse {
        // The press and not the release, and the chord and not every refusal:
        // this machine reads a modifier pressed and released with nothing
        // between the two as an ask of its own — the foreground window's menu —
        // and the chord's modifier is the one it is shown, since a modifier
        // whose release is refused stays down for it (ADR-0013). One tap per
        // press of the chord is enough to be something in between; one per
        // refused key would be a key reaching the foreground on every keystroke
        // typed while the keyboard is the other machine's.
        if !released && is_the_switch(at, answers) {
            answers.show_here(&a_mask());
        }
...
/// What this machine is shown so that a modifier it is holding does not read as
/// one pressed and released with nothing in between.
///
/// A key going down and up, which is something rather than nothing: the machine
/// reads a lone modifier as an ask of its own — the foreground window's menu —
/// and what fails that reading is any key in the middle. Both halves, because a
/// press on its own is a key this machine then holds until something lets go of
/// it.
fn a_mask() -> [Stroke; 2] {
    [
        Stroke {
            key: Shown::AKeyNothingActsOn,
            released: false,
        },
        Stroke {
            key: Shown::AKeyNothingActsOn,
...
/// The modifier shown as held, with that mask behind it.
///
/// Built from [`a_mask`] rather than spelling the two strokes again, because the
/// press is exactly what needs masking: shown on its own, the release the
/// person's finger will provide is the second half of a lone modifier, which is
/// the thing the mask exists to prevent arriving one finger-lift later.
///
/// No release of the modifier's own, for that same reason: nothing is refused
/// once the keyboard is back, so the finger coming off reaches the machine as
/// itself.
fn the_modifier_held() -> [Stroke; 3] {
    let [down, up] = a_mask();
    [
        Stroke {
            key: Shown::TheModifier,
            released: false,
        },
        down,
        up,
...
fn relay(
    ...
    if *request == Request::DryRun {
        ...
        // The clock before the thread, because what it stamps is what that
        // thread reads: one taken afterwards would be behind the first keystroke
        // against it.
        host.start_a_clock();
        if !host.take_input(
            PROBE_TICK,
    ...
    // The clock before the thread, for the reason the dry run takes it first.
    host.start_a_clock();
    if !host.take_input(
        PROBE_TICK,
        RAW_BYTES,
    ...
    loop {
        ...
        loop {
            ...
            if asked == Some(Driving::ThisMachine) {
                ...
                // And this machine told about the modifier the finger is still
                // on, which is the same hazard the other way round: the press
                // that made the chord was refused, so this machine never saw it,
                // and one that believes the modifier is up answers for it that
                // way for as long as the finger stays — the chord's own letter
                // stops being recognised, and the next key typed under it
                // arrives bare. Once, whichever option keys are held: what the
                // machine is asked about is the modifier and not which of them
                // (`docs/platform/windows/hooks-and-raw-input.md`).
                if chording.still_holding().next().is_some() {
                    host.show_here(&the_modifier_held());
                }
