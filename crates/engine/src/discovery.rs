...
pub fn find(service: &str, patience: Duration, host: &mut dyn DiscoveryHost) -> Option<Sink> {
    ...
    // A responder that sent the address along has saved a lookup, and spending
    // one anyway is a question asked of a resolver for an answer already in
    // hand — so which of the two is asked for is decided here (ADR-0006).
    match found.address {
        Some(address) => host.a_sink_at(address, found.port),
        None => host.resolve_discovered(&found.host, found.port),
    }
}
