...
use favjit_host::{Host, NoOutput, OutputReport};
...
/// How long this run lasts, and what happens once it is up.
///
/// Stated as how long rather than as the moment it ends, because the origin
/// [`Instant`] counts from is the host's own (ADR-0006): whoever asks for a
/// bound has a number of seconds and no way to place it on that clock, and
/// placing it is one read of the clock as the run comes up.
///
/// The two are one setting and not a bound with a flag beside it: ending at the
/// moment and hanging at it are two things to do with one moment, so a run given
/// both would be a race whose loser never happened.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub enum Bound {
    /// It ends for its own reasons and no other.
    #[default]
    Whenever,
    /// It stops once this much has passed since it came up.
    Stops(Duration),
    /// It hangs once that has, rather than stopping.
    ///
    /// What ADR-0008 rules out is a process holding this machine's input with
    /// nothing left that would end it, and the only way to find out whether the
    /// watchdog reaches that is a run that reaches it on purpose.
    Wedges(Duration),
}

/// The moment a run was told to end at, on the clock its host reports.
#[derive(Debug, Clone, Copy, Default)]
struct Bounded {
    at: Option<Instant>,
    /// Whether reaching it means hanging rather than stopping.
    wedges: bool,
}

impl Bounded {
    /// Placed by asking the host what time it is now, since that is the only
    /// thing the origin of its clock can be read from.
    fn of(bound: Bound, host: &mut dyn Host) -> Self {
        let (after, wedges) = match bound {
            Bound::Whenever => return Self::default(),
            Bound::Stops(after) => (after, false),
            Bound::Wedges(after) => (after, true),
        };
        let nanos = u64::try_from(after.as_nanos()).unwrap_or(u64::MAX);
        Self {
            at: Some(host.now().saturating_add(Duration::from_nanos(nanos))),
            wedges,
        }
    }

    /// Whether it has come.
    fn up(&self, now: Instant) -> bool {
...
struct Sink {
    ...
    /// What to show a person about each report, where this run was asked for it.
    ///
    /// Absent rather than a flag beside an always-present one, so the run that
    /// delivers carries neither the decoding nor the list of what it wrote: a
    /// keystroke-per-line record kept for a mode nobody asked for would grow for
    /// as long as the run lasts.
    describing: Option<Describing>,
}

impl Sink {
    /// Send what this converts out through that one instead.
    ///
    /// The output swapped and not the sink rebuilt around it, because what this
    /// holds is which keys are down on the keyboards a person is typing on: a
    /// sink made afresh would take the next release for a key it never saw go
    /// down, and a modifier held across the swap would stop being applied to
    /// what is typed under it.
    fn writes_to(&mut self, writing: Box<dyn favjit_host::sink::Injecting>) {
        self.writing = writing;
    }

    /// Start describing what it writes.
    ///
    /// Asked for rather than read off the request, because the two runs that want
    /// it are not one mode: a dry run shows a person what would have gone, and a
    /// replay shows what did — and the run that delivers wants neither, since a
    /// line per keystroke would be a write on the interactive path.
    fn describes(&mut self) {
        self.describing = Some(Describing::default());
    }

    /// Show whatever the last write produced, and nothing where it described
    /// nothing.
    fn say_what_it_wrote(&mut self, host: &mut dyn SinkHost) {
        let Some(describing) = self.describing.as_mut() else {
            return;
        };
        for line in core::mem::take(&mut describing.pending) {
            host.say(format_args!("would send {line}"));
        }
    }

    /// Every report this run wrote, for a caller that wants the bytes as well as
    /// the words.
    pub fn wrote(&self) -> &[(OutputReport, Vec<u8>)] {
        self.describing.as_ref().map_or(&[], Describing::wrote)
    }
    ...
    fn inject(&mut self, injected: Injected, host: &mut dyn SinkHost) {
        ...
        if let Ok(rendered) = rendered {
            ...
            // different remedies.
            let before = host.now();
            code = send(rendered, &mut *self.writing, self.describing.as_mut());
            let after = host.now();
            // After the write and after the reading, so the words a person sees
            // are never ahead of the report they describe and never inside what
            // the two clocks above are measuring.
            self.say_what_it_wrote(host);
            self.latency
                .pipeline
...
fn send(
    rendered: Rendered,
    writing: &mut dyn favjit_host::sink::Injecting,
    mut describing: Option<&mut Describing>,
) -> i32 {
    // Read off the bytes on their way out rather than from what the sink meant to
    // write: a description built from the intention could say one thing while the
    // device was sent another, which is the one thing a run that shows a person
    // its output must not do.
    let wrote = |kind: OutputReport, bytes: &[u8], describing: &mut Option<&mut Describing>| {
        if let Some(describing) = describing.as_deref_mut() {
            describing.took(kind, bytes);
        }
    };
    match rendered {
            ...
            for one in sent {
                ...
                let code = match one {
                    report::Sent::Keyboard(report) => {
                        ...
                        let code = writing.send_report(OutputReport::Keyboard, &bytes);
                        wrote(OutputReport::Keyboard, &bytes, &mut describing);
                        code
                    }
                    report::Sent::Control(control) => {
                        let kind = control_report(control.page);
                        let bytes = control.bytes();
                        let code = writing.send_report(kind, &bytes);
                        wrote(kind, &bytes, &mut describing);
                        code
                    }
                };
                if trouble == 0 {
        ...
        Rendered::Pointing(report) => {
            let bytes = report::pointing(report);
            let code = writing.send_report(OutputReport::Pointing, &bytes);
            wrote(OutputReport::Pointing, &bytes, &mut describing);
            code
        }
    }
}

/// What a run that injects nothing has to show, built from the reports it wrote.
///
/// Here rather than behind the writer a run that shows nothing else is handed:
/// reading a report is favjit's own format and its own tables, so a decoder on
/// the far side of the boundary would be a host holding them — and the words it
/// produced would be a host's own (ADR-0006).
#[derive(Default)]
pub struct Describing {
    /// The report before the last one written, so a line can name what changed
    /// rather than the whole state a report carries.
    last: report::Report,
    /// Each line and how many times it has been produced.
    seen: BTreeMap<String, usize>,
    /// The lines still to be shown, oldest first.
    pending: Vec<String>,
    /// Every report as it was written, for the bytes to be shown afterwards.
    wrote: Vec<(OutputReport, Vec<u8>)>,
}

impl Describing {
    /// One report on its way out.
    fn took(&mut self, kind: OutputReport, bytes: &[u8]) {
        for line in self.lines(kind, bytes) {
            ...
            *n += 1;
            ...
            // is hundreds of times a second, and showing each one would bury the
            // keystrokes this mode exists to show — which is what the count is
            // kept for, since the line itself is the same one every time.
            if kind != OutputReport::Pointing || *n == 1 {
                self.pending.push(line);
            }
        }
        self.wrote.push((kind, bytes.to_vec()));
    }

    fn lines(&mut self, kind: OutputReport, bytes: &[u8]) -> Vec<String> {
        match kind {
            OutputReport::Keyboard => match report::Report::from_bytes(bytes) {
                // Dropped rather than named as unreadable: these are bytes this
                // crate built a moment ago, so a report that will not parse is
                // this file disagreeing with itself and not something a person
                // reading the output can act on.
                None => Vec::new(),
                Some(report) => {
            ...
            OutputReport::Pointing => match favjit_hid::report::pointing_from_bytes(bytes) {
                None => Vec::new(),
                Some(report) => vec![format!(
                    "move buttons={:#b} wheel={}",
                    report.buttons.bits(),
                    report.vertical_wheel != 0 || report.horizontal_wheel != 0
                )],
            },
            // A control page, shown by its byte count rather than decoded: the
            // layout's rules do not reach one, so naming the exact field would be
            // a table kept for a case this mode never sees.
            other => vec![format!("{other:?}: {} byte(s)", bytes.len())],
        }
    }

    /// Every report as it was written, in order.
    pub fn wrote(&self) -> &[(OutputReport, Vec<u8>)] {
        &self.wrote
    }
}

/// The keys and modifiers this report added or let go of since the last one, one
/// line each — read off the usages and the modifier byte rather than assumed,
/// since a live report is exactly the case where a guess and the device
/// disagreeing would go unnoticed.
fn keyboard_diff(before: &report::Report, after: &report::Report) -> Vec<String> {
    ...
    for key in MODIFIERS {
        let bit = report::modifier_bit(key).unwrap_or(0);
...
/// The keyboard-page key this usage names, or the usage itself where no name
/// covers it — read off the same table a report is decoded with, not guessed at.
fn named_usage(usage: u16) -> String {
    match crate::hid::usage::named(crate::hid::page::KEYBOARD_OR_KEYPAD, u32::from(usage)) {
        Some(key) => format!("{key:?}"),
...
fn wait<H: Host + ?Sized>(
    ...
    // The soonest of the three, and `RESPONSIVENESS` on its own where there is
    // neither a timer nor a bound: a wait bounded only by a repeat that is not due
    // is a run that looks at none of the facts below until one is.
    //
    // The bound is one of them rather than only looked at between events, because
    // a wait already begun is not shortened by one: a run given three seconds on a
    // quiet machine would sit in this call until something arrived.
    let deadline = timer.map_or(responsive, |timer| timer.min(responsive));
    let deadline = bound.at.map_or(deadline, |at| deadline.min(at));
...
fn switched_off(host: &mut dyn SinkInputHost, bound: &Bounded) -> Option<Ending> {
    ...
    // The bound is looked at here too, because a run switched off is one this
    // loop would otherwise sit in for as long as it stays off: a bound given on a
    // command line is about the run and not about the converting.
    while !host.switched_on() && !bound.up(host.now()) {
        if host.stop_requested() {
            break;
        }
        wait(host, None, bound, &mut waiting);
    }
    if !host.switched_on() {
...
struct Resolver {
    ...
    /// Every usage nothing named, once each and in the order first seen.
    ///
    /// Kept as well as said, because the two are read at different moments: the
    /// lines go by while the person is still pressing keys, and what they read
    /// afterwards is the list — with the repeats dropped, since a key reports
    /// every time it is pressed and a list padded by that says nothing more.
    unnamed: Vec<(DeviceId, u32, u32)>,
}
        ...
        match crate::hid::usage::named(page, usage) {
            ...
            None => {
                if down {
                    ...
                    // On the press only, like the line above: a release names the
                    // same usage, so keeping both would put every key on the list
                    // twice over.
                    let seen = (device, page, usage);
                    if !self.unnamed.contains(&seen) {
                        self.unnamed.push(seen);
                    }
                }
                None
            }
...
pub fn watch(host: &mut dyn SinkInputHost, bound: Bound) -> Watched {
    // Placed before the wait for the switch, since that wait is one a bound has to
    // be able to end.
    let bound = Bounded::of(bound, host);
    if let Some(off) = switched_off(host, &bound) {
        return Watched::ending(off);
    }
    // The clock before the loop, because what it stamps is what that loop reads:
    // one taken afterwards would be behind the first keystroke against it.
    host.start_a_clock();
    let Some(mut reading) = host.look_for_devices(Box::new(|turning| {
        crate::capture::convert(turning, CAPTURE_POLL)
    })) else {
        return Watched::ending(Ending::NoInput);
    };
    ...
    let ended = loop {
        match wait(host, None, &bound, &mut facts) {
            Some(raw) => {
                resolve(raw, &mut resolver, host, reading.as_mut());
                beating.beat(host);
                if looked.due(host.now()) {
                    if let Some(ended) = ended_meanwhile(host, false, &facts, &bound) {
                        break ending(ended);
                    }
                }
            }
    ...
    Watched {
        ending: ended,
        unnamed: resolver.unnamed,
    }
}

/// What watching the keyboards found.
///
/// The list beside the ending rather than instead of it: a run that could not
/// read the keyboards found nothing, and "nothing was unnamed" and "nothing was
/// read" are the two answers a person acts on differently.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Watched {
    pub ending: Ending,
    /// Every usage nothing named, once each and in the order first seen.
    pub unnamed: Vec<(DeviceId, u32, u32)>,
}

impl Watched {
    /// A run that ended before it read anything.
...
/// What ends the run, once a connection that went has been given its one
/// attempt at coming back (ADR-0008).
///
/// Here and not inside [`ended_meanwhile`], because that reads facts off the
/// machine and this changes what the run is holding: an attempt made there
/// would be a bring-up inside a fact-query, and which of the facts it asked
/// about would then depend on how far the bring-up got (ADR-0006).
///
/// One attempt, since what keeps the keyboards from being held by a service
/// that will not have them is that the holding is bounded: a loop opening
/// connections for as long as one keeps going is a run suppressing input it
/// cannot deliver for as long as that lasts.
fn ended_or_reconnected(
    host: &mut dyn SinkHost,
    sink: &mut Sink,
    beating: &mut Beating,
    back: &mut JustBack,
    ...
    let Some(ended) = ended_meanwhile(host, output_opened, facts, bound) else {
        back.worked();
        return None;
    };
    match ended {
        // A connection opened and gone again before anything crossed it is the
        // service refusing this run rather than a connection that went: another
        // attempt would be the same refusal, and the keyboards would be held
        // through each of them.
        Ended::OutputGone if back.untried() => Some(Ended::OutputGone),
        Ended::OutputGone => match bring_up_the_output(host, beating) {
            Ok(writing) => {
                // Tuned again, because the devices behind this connection are
                // not the ones the properties were put on: the service builds
                // them afresh for whoever connects, so a pointer left alone
                // here is one that moves at the machine's own rate for the rest
                // of the run (ADR-0011).
                tune_the_output(host);
                sink.writes_to(writing);
                back.opened();
                None
            }
            Err(trouble) => {
                say_why_there_is_no_output(trouble, host);
                Some(Ended::OutputGone)
            }
        },
        ended => Some(ended),
    }
...
/// Whether what the run is writing to is a connection it has opened and not yet
/// seen carry anything.
///
/// A flag and not a count of attempts: a close is ordinary here, so a run that
/// had spent its one attempt an hour ago would end on the next one — and a run
/// counting none would go round opening connections a service closes as fast as
/// it hands them over.
#[derive(Debug, Default)]
struct JustBack(bool);
...
impl JustBack {
    ...
    fn untried(&self) -> bool {
        self.0
    }
}

/// Whether this request's whole output is what it would have written.
///
/// The dry run and no other: a run that delivers puts the keystrokes where the
/// person is typing, so a line per report would be a write on the interactive
/// path saying what they can already see on their screen.
fn describing(request: &Request) -> bool {
    matches!(request, Request::DryRun)
}
...
fn ended_meanwhile(
    ...
    // Before the stop is asked about, because the two are not the same answer
    // where the bound is a hang: a run told to hang at a moment has to reach it
    // even on a machine that has gone quiet, and a stop read first would make the
    // one failure ADR-0008 exists to cover the one a test cannot reach.
    //
    // Between two events rather than in front of the wait for one, so nothing is
    // pulled out of the stream and dropped: a run that ends here leaves a trace
    // whose last event is the last one it actually handled, which is what makes
    // the trace worth having about a wedge at all.
    if bound.up(host.now()) {
        return Some(up(host, bound));
    }
    if host.stop_requested() {
...
/// Which refusal a code the machine answered a seizure with is.
///
/// A table here rather than beside the call that produced the code: the two
/// readings send a person looking in different places — one is a run to make
/// again with privilege and the other is another program to close — and saying
/// which is what a person has to do is the run's (ADR-0006). A table kept where
/// the code comes from would be a host deciding that, and one no case could
/// drive.
///
/// The code itself is said alongside, because a machine answering something this
/// table has no reading for still has to leave a person something to look up.
fn refused(code: i32) -> &'static str {
    match code {
        NOT_PRIVILEGED => "not privileged",
        EXCLUSIVE_ACCESS => "something else holds it",
        _ => "refused",
    }
}

/// The two codes IOKit answers a refused open with
/// (`docs/platform/macos/permissions-through-a-signed-bundle.md`).
///
/// Named here and not read out of the platform's own headers, since the value is
/// what crosses the boundary: a host that named them would be answering with its
/// own reading of the failure rather than with what the machine said.
const NOT_PRIVILEGED: i32 = 0xE00002C1u32 as i32;
const EXCLUSIVE_ACCESS: i32 = 0xE00002C5u32 as i32;

/// What a bound that has come round does.
///
/// The words are said here and not by whoever asked for the bound, because a host
/// writes none of its own (ADR-0006) and the run is the one thing that knows the
/// hang was asked for rather than arrived at.
fn up(host: &mut dyn SinkInputHost, bound: &Bounded) -> Ended {
    if !bound.wedges {
        return Ended::AsAsked;
    }
    host.warn(format_args!(
...
pub fn run(
    ...
    // Placed once, here, rather than by each loop below from the same setting: the
    // two would read the clock at different moments, so a run would be given
    // however long the bringing-up happened to take on top of what it asked for.
    let bound = Bounded::of(settings.bound, host);
    let ended = match trace {
        ...
        Some(bytes) => {
            ...
            match bring_up(request, &mut *recorder.host, &bound) {
        ...
        None => match bring_up(request, host, &bound) {
            Err(stopped) => (stopped, Latency::default()),
            Ok(brought) => {
                let mut reading = brought.reading;
...
fn bring_up_the_output(
    ...
    let mut beaten = started;
    let mut status = crate::output::Status::default();
    // One before the wait and one for every [`crate::supervision::BEAT_WITHIN`]
    // inside it, the way every wait longer than that is taken: a supervisor
    // counts from the moment it started this process, so a wait entered without
    // a beat in front of it is a run that says nothing for as long as the wait
    // lasts — and one beat per [`OUTPUT_READY_POLL`] instead would be fifty a
    // second saying what one of them already said (ADR-0008).
    beating.beat(host);
    while !status.ready() {
        ...
        let now = host.now();
        if now.saturating_duration_since(started) >= OUTPUT_WAIT {
            return Err(status.why_not());
        }
        if now.saturating_duration_since(beaten) >= crate::supervision::BEAT_WITHIN {
            beating.beat(host);
            beaten = now;
        }
    }
...
fn bring_up(
    ...
    let (suppress, writing) = match request {
        ...
        Request::Injecting { listen } => {
            let writing = match bring_up_the_output(host, &mut Beating::default()) {
                Ok(writing) => writing,
                Err(trouble) => {
                    say_why_there_is_no_output(trouble, host);
    ...
    // The clock before the loop, for the reason [`watch`] takes it first.
    host.start_a_clock();
...
/// bring up.
///
/// One parameter each rather than a struct holding them: every one is settled
/// before the loop starts and none of them is read together with another, so a
/// struct would be a name for "the arguments" and a reader would have to open it
/// to find out what the loop is given.
...
fn convert(
    ...
    let _ = describe.then(|| sink.describes());
    let mut resolver = Resolver::new(input, exclusive);
    let mut beating = Beating::default();
    let mut facts = Facts::default();
    let mut back = JustBack::default();
    let mut looked = LookedIn::default();
    let ended = loop {
        let timer = sink.current_timer();
        match wait(host, timer, bound, &mut facts) {
            Some(raw) => {
                ...
                if looked.due(host.now()) {
                    if let Some(ended) = ended_or_reconnected(
                        host,
                        &mut sink,
                        &mut beating,
                        &mut back,
...
fn convert_from(
    ...
    host: &mut dyn SinkHost,
    writing: Box<dyn favjit_host::sink::Injecting>,
) -> Vec<(OutputReport, Vec<u8>)> {
    let mut sink = Sink::from_checkpoint(layout, settings.into(), checkpoint, writing);
    // Always, because a replay exists to be read: a reproduction nobody could see
    // the reports of would only say how many events it handled.
    sink.describes();
    let mut beating = Beating::default();
    for &event in events {
        sink.handle(event, host);
...
pub fn replay(
    ...
    let wrote = convert_from(layout, settings, &checkpoint, &events, host, writing);
    Replayed::Events {
        events: events.len(),
        wrote,
    }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Replayed {
    /// The events after that checkpoint, every one of them handled, and every
    /// report that went out while they were.
    ///
    /// The bytes as well as the count, because what a case asserts is what
    /// crossed to the device (ADR-0006) — which a line naming the key it stands
    /// for cannot be turned back into.
    Events {
        events: usize,
        wrote: Vec<(OutputReport, Vec<u8>)>,
    },
...
///
/// What is written is read off the answers this loop already holds rather than
/// out of the sink: a sink that recorded its own intentions could record one
/// thing and do another, and the whole value of a trace is that replaying it
/// cannot disagree with the run.
///
/// Recorded here and not behind a stand-in the calls are routed through: a host
/// trait is the platform's own surface (ADR-0006), so anything of `engine`'s
/// wearing one is `engine` pretending to be a machine — and a pass-through that
/// records nothing buys the appearance of a guarantee without the guarantee.
#[allow(clippy::too_many_arguments)]
fn convert_traced(
    layout: Layout,
    settings: Settings,
    ...
    let ended = loop {
        let timer = sink.current_timer();
        match wait(&mut *recorder.host, timer, bound, &mut facts) {
            Some(raw) => {
                ...
                let Some(event) = resolve(raw, &mut resolver, &mut *recorder.host, reading) else {
                    continue;
                };
                handle_traced(&mut sink, recorder, &mut beating, event);
                if looked.due(recorder.host.now()) {
                    if let Some(ended) = ended_or_reconnected(
                        &mut *recorder.host,
                        &mut sink,
                        &mut beating,
            ...
            None => match due(&mut *recorder.host, timer) {
                Some(event) => {
                    handle_traced(&mut sink, recorder, &mut beating, event);
                    if looked.due(recorder.host.now()) {
                        if let Some(ended) = ended_or_reconnected(
    ...
    sink.release_everything(&mut *recorder.host);
    (ended, sink.latency.with(facts.arrival))
}
...
fn handle_traced(
    ...
    sink.handle(event, &mut *recorder.host);
    ...
    beating.beat(&mut *recorder.host);
}
