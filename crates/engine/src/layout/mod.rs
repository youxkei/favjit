...
impl FromMods {
    ...
    pub(crate) const fn mandatory(mandatory: Modifiers, optional: Modifiers) -> Self {
        Self {
            mandatory,
    ...
    pub(crate) const fn tolerating(optional: Modifiers) -> Self {
        Self {
            mandatory: Modifiers::NONE,
            optional: Optional::Only(optional),
        }
...
impl Layout {
    pub(crate) fn new(rules: Vec<Rule>, dudrack_externals: Vec<DeviceMatch>) -> Self {
        Self {
            rules,
            dudrack_externals,
        }
    ...
    /// Only [`super::hid::tables`] presses a layout against the table this way;
    /// production code resolves one key at a time through [`Self::resolve`].
    #[cfg(test)]
