...
use favjit_host::capture::{Ask, Found, Reading, SinkCapture, SourceCapture, Waiting};
...
pub fn convert(host: &mut dyn SinkCapture, poll: Duration) {
    ...
    let _ = listed.then(|| {
        ...
        let mut reading = drain_devices(host, &mut known, &mut here);
        while reading && !host.over() {
            host.turn_the_loop(poll);
            reading = answer_probes(host);
            answer_what_was_asked(host);
            reading &= drain_devices(host, &mut known, &mut here);
...
fn drain_arrivals(host: &mut dyn SourceCapture) -> bool {
    let mut reading = true;
    while nameable(host.what_is_waiting()) {
        let Some(arrived) = host.take_what_is_waiting() else {
            break;
        };
...
/// Whether the front of what a turn put down is one the run can name.
///
/// Left where it is rather than dropped while the device it came from carries no
/// number: the number arrives on the device drain of this same turn, so the very
/// next drain reads it — and a report thrown away for want of one is a keystroke
/// nobody typed twice.
///
/// Stopping there rather than reading past it: what is behind it came later, and
/// a drain that skipped ahead would put the two on the stream in the other
/// order — which is the one thing the order of this queue exists to hold.
fn nameable(waiting: Option<Waiting>) -> bool {
    waiting.is_some_and(|one| one.numbered.is_some() || !one.from_a_device)
}
...
/// Which of the two listings a finding is looked for on.
#[derive(Clone, Copy)]
enum Listing {
    /// What was already attached when the run started.
    Here,
    /// What has arrived since.
    Arriving,
}

/// Take every registry entry one listing has waiting, keeping the devices this
/// loop has not seen, and say whether it had any.
///
...
fn drain_listing(host: &mut dyn SinkCapture, known: &mut Known, from: Listing) -> (bool, bool) {
    let mut reading = true;
    let mut had_any = false;
    loop {
        // An entry that produced nothing is passed over rather than read as the
        // end of the listing, which is why the two are different answers: one
        // keyboard the machine will not open or will not name would otherwise
        // hide every entry behind it, and what a person sees then is a keyboard
        // they are typing on that nothing converts.
        match one_device(host, known.offer(), from) {
            Stepped::Done => break,
            Stepped::PassedOver => had_any = true,
            Stepped::Found(found) => {
                had_any = true;
                let new = known.keep(found.named);
                // Let go of the second finding of one device rather than keeping
                // it: the handle behind it is one more open on a device already
                // open, and a seize on that one takes hold of nothing.
...
/// What one step through a listing produced.
enum Stepped {
    /// The listing has nothing more waiting.
    Done,
    /// An entry that produced no device to read.
    PassedOver,
    /// One device, under the name the machine gave it.
    Found(Found),
}

/// Take every registry entry either listing has waiting.
///
/// The arrivals on every turn, and never only the listing that still has
/// something on it: IOKit re-arms its telling only once the arrivals iterator
/// has been drained, so a turn that read the other one instead is the last
/// arrival this run ever hears about — and a keyboard plugged in after the run
/// started would go unconverted for as long as the run lasts.
///
/// What was already here only while it still answers, and `here` is how a later
/// turn knows it no longer does: that listing is a snapshot the registry took
/// once, so a turn that kept reading it would ask the machine for findings that
/// cannot arrive.
fn drain_devices(host: &mut dyn SinkCapture, known: &mut Known, here: &mut bool) -> bool {
    let (from_here, had_any) = match *here {
        true => drain_listing(host, known, Listing::Here),
        false => (true, false),
    };
    *here = had_any;
    let (arriving, _) = drain_listing(host, known, Listing::Arriving);
    from_here & arriving
}
...
fn one_device(host: &mut dyn SinkCapture, as_this: DeviceId, from: Listing) -> Stepped {
    let found = match from {
        Listing::Here => host.next_device_here(),
        Listing::Arriving => host.next_arriving_device(),
    };
    ...
    Stepped::Found(Found { named, found: told })
}
...
mod tests {
    ...
    struct Platform {
        ...
        lists: bool,
        found: Vec<u64>,
        /// What arrives after the run started, each under the turn it shows up
        /// on — the turn and not a count, so that a case about a keyboard
        /// plugged in later says when rather than how many drains ago.
        arriving: Vec<(usize, u64)>,
    ...
    impl SinkCapture for Platform {
        ...
        fn next_arriving_device(&mut self) -> Option<favjit_host::Handed> {
            let at = self
                .arriving
                .iter()
                .position(|(turn, _)| *turn <= self.turns);
            at.map(|at| favjit_host::Handed(self.arriving.remove(at).1))
        }
    ...
    struct Watching {
        ...
        found: Vec<u64>,
        delivered: Vec<EventKind>,
        /// What a turn produced, oldest first, each under whether it came from a
        /// device and the number the run has given that device.
        waiting: Vec<(Waiting, EventKind)>,
    ...
    impl SourceCapture for Watching {
        ...
        fn what_is_waiting(&mut self) -> Option<Waiting> {
            self.waiting.first().map(|(waiting, _)| *waiting)
        }

        fn take_what_is_waiting(&mut self) -> Option<EventKind> {
            (!self.waiting.is_empty()).then(|| self.waiting.remove(0).1)
        }
    }
    ...
    #[test]
    fn a_keyboard_plugged_in_after_the_run_started_is_taken() {
        // The arrivals are drained on every turn and not only while the listing
        // of what was already here still answers: IOKit re-arms its telling once
        // that iterator has been drained, so a turn that read the other one
        // instead would be the last arrival this run ever heard about — and this
        // keyboard would go unconverted for as long as the run lasts.
        let mut platform = Platform::with(vec![7]);
        platform.arriving = vec![(2, 9)];
        ...
        assert_eq!(
            ...
            "the one already here and the one that arrived on a later turn"
        );
    }
    ...
    fn what_a_turn_could_not_name_is_delivered_after_the_device_it_came_from() {
        ...
        let mut watching = Watching::with(vec![7]);
        watching.waiting = vec![(named_by(DeviceId(0)), EventKind::Probe)];
    ...
    /// One waiting under a number the run has already given out.
    fn named_by(device: DeviceId) -> Waiting {
        Waiting {
            from_a_device: true,
            numbered: Some(device),
        }
    }

    /// One waiting from a device the run has numbered nothing for yet.
    fn unnamed() -> Waiting {
        Waiting {
            from_a_device: true,
            numbered: None,
    ...
    /// One waiting that came from no device at all.
    fn from_no_device() -> Waiting {
        Waiting {
            from_a_device: false,
            numbered: None,
        }
    ...
    #[test]
    fn a_report_whose_device_has_no_number_waits_rather_than_going() {
        // A report is what says there is a device, so the first one always
        // arrives before the number: dropping it would lose the keystroke that
        // announced the keyboard, and it is the number that is late rather than
        // the report that is unwanted.
        let mut watching = Watching::with(Vec::new());
        watching.waiting = vec![(unnamed(), EventKind::Probe)];
        ...
        assert_eq!(
            watching.delivered,
            Vec::new(),
            "nothing goes while the number is not there"
        );
        assert_eq!(
            watching.waiting.len(),
            1,
    ...
    #[test]
    fn what_is_behind_a_report_with_no_number_waits_with_it() {
        // The order is the whole of what this queue is for: reading past the
        // front would put what came later on the stream first, so a keystroke
        // typed after the one that announced the keyboard would be converted
        // before it.
        let mut watching = Watching::with(Vec::new());
        watching.waiting = vec![
            (unnamed(), EventKind::Probe),
            (from_no_device(), EventKind::AskedToStop),
        ];
        ...
        assert_eq!(
            watching.delivered,
            Vec::new(),
            "the one behind it names no device and still waits its turn"
        );
    }

    #[test]
    fn what_came_from_no_device_goes_without_a_number() {
        // Nothing to number: what the machine said about itself carries no
        // device, so a drain that waited for one would hold a stop nobody
        // could ever hand a number to.
        let mut watching = Watching::with(Vec::new());
        watching.waiting = vec![(from_no_device(), EventKind::AskedToStop)];

        relay(&mut watching, Duration::from_millis(1), true);

        assert_eq!(watching.delivered, vec![EventKind::AskedToStop]);
    }
