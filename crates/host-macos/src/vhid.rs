...
/// The one frame this file spells, which is the one it also fills in the body
/// of: the frames a run answers with are composed where the arriving ones are
/// read (`favjit_engine::output`), and a request's own body is this crate's
/// because the numbers inside it are.
const REQUEST: u8 = 4;
...
impl OutputHost for Serving {
    ...
    fn put_a_frame(&mut self, bytes: &[u8]) -> i32 {
        send(&self.connection.writer, bytes)
    }
...
mod tests {
    ...
    fn what_arrived_is_what_lands_in_the_buffer() {
        let (mut serving, mut writing) = connected(Duration::from_millis(10));
        // Written out rather than composed: what is under test is a read of a
        // real socket, and a frame built by the code that frames them would
        // assert that code against itself.
        writing.write_all(&[0, 0, 0, 1, 0]).expect("written");
        let mut into = [0u8; 5];

        assert_eq!(serving.read_some(&mut into), Took::Bytes(5));
        assert_eq!(into, [0, 0, 0, 1, 0]);
    }
