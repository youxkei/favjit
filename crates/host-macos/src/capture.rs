...
/// Keep the iterator devices come off, and say whether the machine gave one.
fn came_off(iterator: &mut IoIterator, found: Option<IoIterator>) -> bool {
    *iterator = found.unwrap_or(0);
    found.is_some()
}
...
struct Capture {
    ...
    ready: Vec<Ready>,
    gone: Vec<Departed>,
    /// The iterator the devices already attached come off.
    here: IoIterator,
    /// The iterator arriving devices come off.
    ///
    /// Kept apart from the one above rather than the two being read in turn
    /// from one call, because which of them a turn reads is `engine`'s to
    /// decide and not this thread's (ADR-0006).
    arriving: IoIterator,
...
impl Capture {
    ...
    /// The next service the devices already attached come off.
    fn next_here(&mut self) -> Option<Handed> {
        a_service(unsafe { IOIteratorNext(self.here) })
    }

    /// The next service an arrival comes off.
    fn next_arriving(&mut self) -> Option<Handed> {
        a_service(unsafe { IOIteratorNext(self.arriving) })
    }
    ...
    /// Keep the iterator arrivals come off, and say whether IOKit gave one.
    fn keep_the_arrivals(&mut self, hotplug: IoIterator) -> bool {
        came_off(&mut self.arriving, there_is_a_service(hotplug))
    }

    /// Keep the iterator the devices already here come off, and say whether
    /// the registry gave one.
    fn look_here(&mut self) -> bool {
        came_off(&mut self.here, every_hid_device())
    }
...
impl favjit_host::capture::SinkCapture for Turning {
    ...
    fn ask_to_be_told_about_devices(&mut self) -> bool {
        ...
        the_capture(self.capture).keep_the_arrivals(hotplug)
    }
    ...
    fn next_device_here(&mut self) -> Option<Handed> {
        the_capture(self.capture).next_here()
    }

    fn next_arriving_device(&mut self) -> Option<Handed> {
        the_capture(self.capture).next_arriving()
    }
