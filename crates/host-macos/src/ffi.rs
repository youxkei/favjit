...
// answers with, which is `favjit_engine::hid`'s — what this file is for is the C API
...
// The properties a pointing device's feel lives in are not named here. Which one
// holds a factor, and which of them are `IOFixed` 65536ths, is what a run asks
// for by name — a copy beside the call that writes it would be the same table in
// two places, and only one of them where the suite reads it (ADR-0006).
...
/// API favjit does not use, since output goes out as a device (`docs/platform/macos/output-through-a-virtual-hid-device.md`).
pub const HID_REQUEST_LISTEN: u32 = 1;

pub const HID_ACCESS_GRANTED: u32 = 0;
pub const HID_ACCESS_DENIED: u32 = 1;

// CoreGraphics is not linked. Its event-access preflights answer for the window
// server, and output goes out as a device instead (`docs/platform/macos/output-through-a-virtual-hid-device.md`) — so a run that
// asked them would be reporting a permission nothing here uses
