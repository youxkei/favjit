...
pub struct Region {
    ...
    mapping: Mapping,
    /// Kept so the mapping and the descriptor go together, and closed with it.
...
pub fn passed_down(variable: &str) -> Option<c_int> {
    a_descriptor(std::env::var(variable))
}

/// The descriptor a variable holds, and none where it holds anything else.
///
...
/// number, and reading it as one beside the asking would be a turning there
/// (ADR-0006).
fn a_descriptor(value: Result<String, std::env::VarError>) -> Option<c_int> {
    value.ok().and_then(|fd| fd.parse().ok())
}
