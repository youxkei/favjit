...
use favjit_engine::sink::{self, Bound, Ending};
...
fn watching_reads_however_much_is_typed_without_a_way_to_inject_it() {
    ...
    assert_eq!(
        sink::watch(&mut mac, Bound::Whenever).ending,
        Ending::Converted
    );
}
...
fn watching_takes_the_keyboards_without_taking_them_exclusively() {
    ...
    sink::watch(&mut mac, Bound::Whenever);
...
#[test]
fn every_usage_nothing_named_comes_back_once() {
    // Once each, because a key reports every time it is pressed and what this
    // mode produces is the list a person adds to the table: the same usage on
    // four lines would be the answer padded with how often they leaned on it.
    //
    // Answered rather than only said as they arrive, for the same reason: the
    // lines go by while the person is still pressing keys, and the list is what
    // they read afterwards.
    const OTHER: u32 = 0x69;
    let mut mac = SimHost::new();
    mac.attach_built_in(BUILT_IN);
    for usage in [UNNAMED, OTHER, UNNAMED] {
        mac.script(EventKind::HidValue {
            device: BUILT_IN,
            page: favjit_hid::page::KEYBOARD_OR_KEYPAD,
            usage,
            stamp: 1,
    ...
    let watched = sink::watch(&mut mac, Bound::Whenever);

    assert_eq!(
        watched.unnamed,
        vec![
            (BUILT_IN, favjit_hid::page::KEYBOARD_OR_KEYPAD, UNNAMED),
            (BUILT_IN, favjit_hid::page::KEYBOARD_OR_KEYPAD, OTHER),
        ],
        "in the order they were first seen, and the repeat dropped"
    );
}

#[test]
fn a_usage_the_table_names_is_not_in_that_list() {
    // The list exists to say what is missing from the table, so a key the table
    // already names would be a person sent looking for something that is there.
    let mut mac = SimHost::new();
    mac.attach_built_in(BUILT_IN).tap(BUILT_IN, Key::K);

    let watched = sink::watch(&mut mac, Bound::Whenever);

    assert_eq!(watched.unnamed, Vec::new());
}
