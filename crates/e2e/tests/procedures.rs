...
use favjit_host::source::{Shown, Stroke};
...
#[test]
fn refusing_the_chord_masks_the_modifier_it_was_made_with() {
    // The modifier is not refused — a modifier whose release is refused stays
    // down for this machine (ADR-0013) — so refusing the chord's letter leaves
    // the machine shown that modifier pressed and released with nothing in
    // between, which is what it opens the foreground window's menu on. A key
    // nothing acts on, tapped in the middle, is what that test fails on.
    let mut host = reading();
    host.refuse(Suppressing::TheSwitch);
    host.with_the_modifier_down(true);

    // The strokes and not that masking happened, because the order is the whole
    // of it: what stops the modifier reading alone is a key going down and up
    // between its own two halves, and a machine handed the strokes one at a time
    // could not be asked whether it got them in that order.
    ...
    for at in [TO_THE_SINK, BACK_HERE] {
        assert_eq!(
            answered(&host, a_key(at)).shown,
            vec![vec![inert(false), inert(true)]],
            "the chord at {at:?} is refused, so a key nothing acts on is tapped"
        );
    }
    assert_eq!(
        answered(&host, a_release(TO_THE_SINK)).shown,
        Vec::<Vec<Stroke>>::new(),
        "the release is refused too, and one press masks once"
    );
    assert_eq!(
        answered(&host, a_key(AN_ORDINARY_KEY)).shown,
        Vec::<Vec<Stroke>>::new(),
        "a key that carries on leaves the modifier nothing to be read alone from"
    );
}
