...
fn every_event_is_answered_with_one_heartbeat() {
    ...
    // The one in front of the bring-up's wait on the output device, then one
    // device attach, then two presses and two releases.
    assert_eq!(host.heartbeats().len(), 6);
}
...
fn a_probe_produces_a_heartbeat_and_nothing_else() {
    ...
    // Two events, two heartbeats: the attach and the probe — behind the one in
    // front of the bring-up's wait on the output device.
    assert_eq!(host.heartbeats().len(), 3);
...
fn a_probe_answers_while_a_key_is_held() {
    ...
    // The bring-up's wait on the output device, the attach, two probes, and the
    // press and release of each of the two keys.
    assert_eq!(host.heartbeats().len(), 8);
