...
use favjit_engine::source::{run, Request};
...
/// The keyboard on this machine, by the vendor and product a rule names it
/// with.
...
fn a_run_that_is_not_relaying_reaches_for_nothing_outside_the_machine() {
    ...
    run(&Request::DryRun, false, &mut host, None);
...
fn a_relaying_run_takes_the_keyboards_once_and_gives_them_back() {
    ...
    // both screens.
    // Once the keyboard has been asked for, which is what a relaying run waits to be
    // told before it takes anything (ADR-0013).
    let mut host = SimSource::new();
    ...
    run(&Request::Relaying, false, &mut host, None);
