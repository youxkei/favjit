...
use favjit_host::source::{Driving, Shown, Stroke};
...
#[test]
fn coming_back_tells_this_machine_about_the_option_the_chord_was_made_with() {
    // The mirror of what the sink is told going over, and the direction nothing
    // else covers: the chord is made with option, its press was refused along
    // with everything else, so this machine never saw option go down. Coming
    // back with the finger still on it, the machine believes option is up — the
    // chord's own letter stops being recognised, so its repeats and its release
    // reach whatever has the foreground, and the next key typed under that
    // finger arrives there bare.
    let mut host = sending();
    host.press(KEYBOARD, Key::LeftOption);
    host.press(KEYBOARD, SWITCH_BACK);
    ...
    // The modifier shown as held, and the mask behind it: a press whose only
    // company is the release the finger will provide is the lone tap this
    // machine reads as an ask of its own.
    let inert = |released| Stroke {
    ...
    assert_eq!(
        ...
        "the option the chord was made with, once, masked"
    );
}

#[test]
fn coming_back_with_nothing_held_tells_this_machine_nothing() {
    // A chord let go of cleanly before it is pressed leaves nothing for this
    // machine to be told about, and a press it was not shown is a modifier it
    // then holds until something releases it.
    let mut host = sending();
    host.press(KEYBOARD, Key::LeftOption);
    host.release(KEYBOARD, Key::LeftOption);
    ...
    assert_eq!(host.shown_here(), Vec::<Vec<Stroke>>::new());
}
