...
use favjit_engine::sink::{self, InputConfig};
...
use favjit_host_sim::{identity, SimHost, SimLink, SimSource, SimWatchdog, SINK_SEED};
...
/// The keyboard on the machine input is forwarded from, by the vendor and
/// product a rule names it with.
const FORWARDED: (u16, u16) = (1234, 5678);
...
fn supervise(beats: &[favjit_engine::Instant]) -> (Supervised, SimWatchdog) {
...
fn all_four_run_and_neither_watchdog_takes_a_keyboard_away() {
    ...
    // Asked for the keyboard first, because that is what puts a run into the state a
    // watchdog has something to protect: before the ask nothing is refused (ADR-0013).
    windows.asked_for(Driving::TheSink);
    windows.pinned_to(identity(SINK_SEED));
    windows.attach_external(KEYBOARD, FORWARDED.0, FORWARDED.1);
    windows.probe();
    windows.tap(KEYBOARD, Key::International3);
    windows.probe();
    source::run(&source::Request::Relaying, false, &mut windows, None);
