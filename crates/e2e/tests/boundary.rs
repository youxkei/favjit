...
#[test]
fn a_host_trait_is_implemented_only_in_a_host_crate() {
    // Every rule below reads [`HOSTS`] and nothing else, so implementing one of
    // these traits anywhere else is a host the rules do not see — and what that
    // gets past is not a technicality: a deadline read off the clock beside the
    // answer of another call, a count of what a call answered, words of the
    // host's own. Each of those is a finding in a host crate and silence in any
    // other.
    //
    // Read off what `host` declares rather than from a list beside this, for the
    // reason the platform's own names are: a list would go stale as traits are
    // added, and a stale one makes this rule pass by failing to recognise an
    // implementation.
    let traits = host_traits();
    assert!(
        traits.contains("SinkInputHost") && traits.contains("SourceCapture"),
        "the host traits have to be read for this rule to recognise an implementation; found {} \
         names",
        traits.len()
    );
    let mut found = Vec::new();
    for crate_name in crates_named() {
        if HOSTS.contains(&crate_name.as_str()) || crate_name == "host" || crate_name == SIMULATOR {
            continue;
        }
        for (file, parsed) in source_of(&crate_name) {
            walk_items_built(&parsed.items, &mut |item| {
                if let syn::Item::Impl(block) = item {
                    ...
                    if implemented.is_some_and(|name| traits.contains(&name)) {
                        found.push(format!(
                            "{crate_name}/{file}::{}",
                            named_type(&block.self_ty)
                        ));
                    }
                }
            });
    ...
    hold(
        "a host trait is implemented only in a host crate (ADR-0006: the rules below read the \
         host crates, so one implemented elsewhere is a host nothing checks)",
        found,
        HOSTS_ELSEWHERE,
    );
}

/// Both machines in one process, which is what the suite drives (ADR-0007).
///
/// Whole rather than type by type, because there is no platform behind any of it:
/// what it plays is whatever a case needed, and a rule over its shape would be a
/// rule about the assertions. Nothing ships with it — `favjit-e2e` is the one
/// crate that depends on it — so a host hidden here is a host no run reaches.
const SIMULATOR: &str = "host-sim";

/// Every host implemented outside a host crate, and why it is one.
const HOSTS_ELSEWHERE: &[(&str, &str)] = &[
    (
        "bin-watchdog/unix.rs::Unix",
        "the supervisor's own platform half, which its `Cargo.toml` states the reason for: what \
         it asks of a machine is a pipe and a way to end a process, and reaching those through \
         the crate that also holds the capture and the injection would put a platform's whole \
         surface inside the program that must stay trivial (ADR-0008)",
    ),
    (
        "bin-watchdog/windows.rs::Windows",
        "the same on the other machine",
    ),
];

/// Every crate under the workspace's `crates` directory, by name.
fn crates_named() -> Vec<String> {
...
/// Every trait the `host` crate declares, which is what a host is.
fn host_traits() -> BTreeSet<String> {
    let mut found = BTreeSet::new();
    for (_, parsed) in source_of("host") {
        walk_items(&parsed.items, &mut |item| {
            if let syn::Item::Trait(declared) = item {
                found.insert(declared.ident.to_string());
            }
        });
    }
    found
...
const REACHES_THE_MACHINE: [&str; 40] = [
    // The environment and the filesystem, which a host reaches through `std`
    // rather than through a declaration of its own: reading a variable this
    // process was started with and asking whether a path is there are the
    // machine answering, the same way a socket call is, and a list that named
    // only the declarations would measure a host on which of its calls happen
    // to need one.
    "var",
    "exists",
    "write",
    // The clock, which is the machine answering the same way a socket call is:
    // what time it is now and how long ago something was are not values a host
    // holds, and a list that named only the declarations would measure a host on
    // which of its reads happen to need one.
    "now",
    "elapsed",
    "read_to_string",
    "read_exact",
    ...
    "spawn",
    "sleep",
    // This thread handed to the machine, which is what ends it: a body that
    // parked would otherwise read as one that did nothing, since nothing about
    // the call's shape says the thread does not come back.
    "park",
    "getrandom",
    "connect",
    "bind",
    ...
    // A name resolved, which is the machine asking the network on this run's
    // behalf and the longest wait any of these takes.
    "to_socket_addrs",
    "as_raw_fd",
    "into_raw_fd",
    "from_raw_fd",
...
const MESSAGES: &[(&str, &str)] = &[
    ...
    // And `Host::say`, which is that same operation for what a run shows about
    // its own output rather than about a pairing.
    ("host-macos/lib.rs::say", KEEPS_THE_LOG),
    ("host-windows/lib.rs::say", KEEPS_THE_LOG),
];
...
fn typed_receiver(
    ...
    match receiver {
        syn::Expr::Field(field) => {
            // A tuple field under the number it is spelled by, the way
            // [`field_types`] keys one: a wrapper's `self.0` resolving to
            // nothing would make every method forwarding through one read as a
            // body that calls nothing.
    ...
    for (file, parsed) in host_files() {
        ...
        walk_items(&parsed.items, &mut |item| {
            if let syn::Item::Struct(declared) = item {
                ...
                // A tuple struct's fields under the numbers they are spelled by,
                // which is how a body reaches them: keyed on `field.ident` alone,
                // a wrapper's `self.0` would resolve to nothing, and a method
                // forwarding through one would read as a body that calls nothing
                // at all.
                for (at, field) in declared.fields.iter().enumerate() {
                    ...
                    fields.insert(name, innermost(&field.ty));
                }
            }
        });
    }
...
enum Shape {
    ...
    ATurningBesideACall,
    /// A function that neither reaches the machine nor is registered as a
    /// conversion.
    ///
    /// Whatever it holds, and not only a turning: a body with no branch in it
    /// still decides — how many of something goes out, and in what order — and
    /// a rule that asked only about turnings would let a sequence built out of
    /// constants through, which is a decision written where nothing reads it.
    NeitherACallNorAConversion,
...
impl Shape {
    fn said(self) -> &'static str {
        match self {
            ...
            Self::NeitherACallNorAConversion => {
                "neither a call into the machine nor a registered conversion"
            }
...
impl Shaped<'_> {
    ...
    fn leave(&mut self) {
        ...
        // One mould and nothing else: take the arguments, convert them, make the
        // one call, convert what comes back. The turning is refused beside the
        // call and allowed in the conversion, because a conversion that has to
        // ask a value what it holds branches on the platform's own calling
        // convention and not on anything favjit decided — and what keeps that
        // from being a way through is the last arm: a function that makes no
        // call and is on no list cannot exist, so a decision cannot hide in one
        // by having no branch in it.
        let shape = match (calls, turnings, registered) {
            ...
            (0, _, false) => Some(Shape::NeitherACallNorAConversion),
            _ => None,
        };
...
const ASKS_THE_MACHINE_NOTHING: [&str; 24] = [
    ...
    "CFRunLoopGetCurrent",
    // A reference given back, which is the disposal half of the copy a property
    // call made: a conversion that found the value was not the type it wanted
    // has to let go of it before answering with nothing, and counting that would
    // make every such conversion a call.
    "CFRelease",
    "CFGetTypeID",
    "CFNumberGetTypeID",
    "CFStringGetTypeID",
...
/// What a name on the platform's side is read as on the run's, and back.
///
/// One sentence for the whole group, because it is one conversion spelled once
/// per thing being converted: a handle the machine gave, read as the number the
/// run carries it as, or the other way round.
const A_HANDLE_EITHER_WAY: &str = "a handle the machine gave, read as the number the run carries \
                                   it as, or that number read back as the handle a call takes";

/// The lists a bare callback writes what it saw into, read or written by name.
///
/// One sentence for the group, for [`A_HANDLE_EITHER_WAY`]'s reason. The state
/// is on the machine's side because a callback is a bare function pointer with
/// nowhere to hang a context off — the same fact [`NOWHERE_TO_HANG_IT`] names
/// for the statics themselves — and every one of these turns what the run named
/// into what those lists hold, or back.
const THE_CALLBACKS_OWN_LISTS: &str = "what a bare callback put down, read as the value the run \
                                       named it by, or that value read back into the lists \
                                       nothing else can reach";

/// A value the host already holds, read as what the run asks it for.
///
/// One sentence for the group, for [`A_HANDLE_EITHER_WAY`]'s reason. Nothing is
/// asked of the machine in any of them, because the answer was taken when the
/// thing was made: a flag another thread put down, a number handed in at
/// start-up, or — for the run that takes no devices — the absence a mode is.
const A_COMPOSED_LINE_PUT_SOMEWHERE: &str =
    "a line the run composed, put where the person who started it reads it: the words and which \
     of them are worth saying are the run's, and what is left here is one write through a macro \
     the rule above cannot see as a call";

const WHAT_THE_HOST_ALREADY_HOLDS: &str = "a value this host already holds, read as what the run \
                                           asks it for: the machine was asked when the thing was \
                                           made, not here";
...
const CONVERSIONS: &[(&str, &str)] = &[
    ("host-macos/capture.rs::the_service", A_HANDLE_EITHER_WAY),
    (
        "host-macos/link.rs::identity_path",
        "a directory read as the file inside it this machine keeps an identity in",
    ),
    (
        "host-macos/link.rs::authorized_path",
        "the same directory read as the file the paired machine's key is kept in",
    ),
    (
        "host-macos/link.rs::IdentityFile::default",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/link.rs::IdentityFile::fill",
        A_HANDLE_EITHER_WAY,
    ),
    (
        "host-macos/link.rs::Link::warn",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/link.rs::Talk::flush_answer",
        "a stream this machine writes straight through, read as a flush that went: there is \
         nothing buffered for one to push",
    ),
    (
        "host-macos/link.rs::Talk::sending_it_away",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    ("host-macos/link.rs::went", A_HANDLE_EITHER_WAY),
    (
        "host-macos/pairing.rs::Pairing::new",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    ("host-macos/pairing.rs::Pairing::fill", A_HANDLE_EITHER_WAY),
    (
        "host-macos/pairing.rs::Pairing::show",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/pairing.rs::Pairing::drop",
        "the socket given back, which is the only thing that type is for",
    ),
    ("host-macos/pairing.rs::went", A_HANDLE_EITHER_WAY),
    ("host-macos/pairing.rs::Exchange::fill", A_HANDLE_EITHER_WAY),
    (
        "host-macos/pairing.rs::Exchange::flush_answer",
        "a stream written straight through, read as a flush that went",
    ),
    (
        "host-macos/pairing.rs::Exchange::flush_sealed_key",
        "the same reading of the other thing this end writes",
    ),
    (
        "host-windows/link.rs::IdentityFile::new",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/link.rs::Link::new",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/link.rs::Link::use_fixed",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/link.rs::Link::bind_discovery",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/link.rs::Link::connect",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/link.rs::Open::flush_first_message",
        "a stream written straight through, read as a flush that went",
    ),
    (
        "host-windows/link.rs::Open::keep",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    ("host-windows/link.rs::went", A_HANDLE_EITHER_WAY),
    (
        "host-windows/link.rs::identity_path",
        "a directory read as the file inside it this machine keeps an identity in",
    ),
    (
        "host-windows/link.rs::sink_path",
        "the same directory read as the file the sink it sends to is written down in",
    ),
    (
        "host-windows/link.rs::under",
        "the two variables read as the directory favjit's own files sit in, and where under them \
         it sits",
    ),
    (
        "host-windows/link.rs::a_path",
        "the path a variable holds, and nothing where it holds nothing",
    ),
    (
        "host-windows/pairing.rs::Pairing::new",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/pairing.rs::Pairing::connect",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/pairing.rs::Offer::flush_offer",
        "a stream written straight through, read as a flush that went",
    ),
    (
        "host-windows/pairing.rs::Offer::flush_sealed_key",
        "the same reading of the other thing this end writes",
    ),
    ("host-windows/pairing.rs::went", A_HANDLE_EITHER_WAY),
    (
        "host-macos/lib.rs::MacOsHost::stop_on",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::output_connected",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::stop_requested",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::is_supervised",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::warn",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::say",
        A_COMPOSED_LINE_PUT_SOMEWHERE,
    ),
    (
        "host-macos/lib.rs::MacOsHost::wanted_pointer_feel",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::output_vendor",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::MacOsHost::instead_of_the_output",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::host",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::stop_on",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::output_connected",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::stop_requested",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::is_supervised",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::warn",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::say",
        A_COMPOSED_LINE_PUT_SOMEWHERE,
    ),
    (
        "host-macos/lib.rs::DryRun::read",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::make_directory",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::open",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    ("host-macos/lib.rs::refuses", WHAT_THE_HOST_ALREADY_HOLDS),
    (
        "host-macos/lib.rs::DryRun::wanted_pointer_feel",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::output_vendor",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::open_event_system",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::open_simple_event_system",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::reach_the_output",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::instead_of_the_output",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::bind_link",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::run_alongside",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-macos/lib.rs::DryRun::run_output_alongside",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::until",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::say",
        A_COMPOSED_LINE_PUT_SOMEWHERE,
    ),
    (
        "host-windows/lib.rs::WindowsHost::refused",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::drop",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::bind_discovery",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::answer_procedures_with",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::has_a_sink_to_look_for",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::use_fixed_sink",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::suppress",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::there_is_somewhere_to_show_it",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::is_supervised",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    (
        "host-windows/lib.rs::WindowsHost::warn",
        WHAT_THE_HOST_ALREADY_HOLDS,
    ),
    ("host-macos/capture.rs::let_go", A_HANDLE_EITHER_WAY),
    (
        "host-macos/capture.rs::this_run_loop",
        "this thread's own run loop, which every scheduling call takes as its second argument: a \
         property of the caller and not of any device",
    ),
    (
        "host-macos/capture.rs::timebase",
        "the ratio this machine names for turning its own ticks into nanoseconds",
    ),
    (
        "host-macos/capture.rs::describe",
        "the properties a device already in hand answered, read as what the run calls a keyboard",
    ),
    ("host-macos/capture.rs::went", A_HANDLE_EITHER_WAY),
    (
        "host-macos/capture.rs::Capture::describe",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::let_go_of_the_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::how_many_elements_of",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::has_an_element_at",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::let_go_of_the_elements",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::let_go_of_the_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::over",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::describe",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::keep",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::let_go_of_the_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::forget",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::next_ask",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::keep_what_was_seized",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::how_many_elements_of",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::has_an_element_at",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::let_go_of_the_elements",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::keep_the_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::let_go_of_the_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::answer_the_ask",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::next_thing_with_values",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::nothing_more_of",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::done_with",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::next_gone",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::its_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Turning::forget_it",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Reading::take_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Reading::read_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Reading::next_held_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Reading::forget_what_was_given_back",
        THE_CALLBACKS_OWN_LISTS,
    ),
    ("host-windows/suppress.rs::held_word", A_HANDLE_EITHER_WAY),
    (
        "host-windows/suppress.rs::keys_go_to",
        THE_CALLBACKS_OWN_LISTS,
    ),
    ("host-windows/suppress.rs::take", THE_CALLBACKS_OWN_LISTS),
    ("host-windows/suppress.rs::refused", THE_CALLBACKS_OWN_LISTS),
    (
        "host-windows/suppress.rs::the_chord_is_at",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::answering",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::answer_with",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::refusing",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::the_chord",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::keys_go_somewhere",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::held_here",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::now_held_here",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::let_go_of_here",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::one_pointer_refused",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/suppress.rs::Called::end_it_here",
        "the number this platform answers a procedure with to end an event, which is what ending \
         one is here",
    ),
    (
        "host-windows/suppress.rs::as_inputs",
        "the strokes a run asked for, as the structures this machine takes them in",
    ),
    (
        "host-windows/suppress.rs::one_input",
        "one of them, likewise",
    ),
    (
        "host-windows/suppress.rs::a_virtual_key",
        "which key this machine calls each of the two a run can name",
    ),
    (
        "host-windows/suppress.rs::going",
        "which half of a stroke it is, as the flag the call takes",
    ),
    (
        "host-windows/suppress.rs::went",
        "what a post answered, read as whether it went",
    ),
    (
        "host-windows/suppress.rs::keyboard_hook",
        "the three words this machine calls a procedure with, read as the event they hold and the \
         call they came in — the whole of what the run is handed, since a bare function pointer \
         is given nothing else",
    ),
    (
        "host-windows/suppress.rs::mouse_hook",
        "the same three words read the same way, for the other of the two procedures",
    ),
    (
        "host-windows/capture.rs::watch",
        "what a run asked for, read as the state the thread holding the hooks keeps: the chord's \
         positions, the clock the stamps go on, and a buffer the size a run said",
    ),
    (
        "host-windows/capture.rs::this_module",
        "this process's own module, read as the handle a window class takes: a property of the \
         caller and not of any device",
    ),
    (
        "host-windows/capture.rs::Turning::over",
        "the flag that thread put down, read as whether the run is over",
    ),
    (
        "host-windows/capture.rs::Turning::send_the_keys_to_that_window",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::Turning::state_the_chord",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::Turning::next_device",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::Turning::what_is_waiting",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::Turning::take_what_is_waiting",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::Watching::hand_up",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-windows/capture.rs::what_is_attached",
        "nothing, read as the listing a run asks how long it is: this machine names no keyboard, \
         so the list it answers with is empty and has no call behind it",
    ),
    (
        "host-windows/capture.rs::started",
        "what a spawn answered, read as whether the thread is there",
    ),
    (
        "host-windows/capture.rs::window_class",
        "the class a window is made from, filled in as the structure the call takes: this module \
         and a default procedure, and nothing drawn",
    ),
    (
        "host-windows/capture.rs::the_timers_turn",
        "a message read as whether it is the probe timer's, by the number the timer was set under",
    ),
    (
        "host-windows/capture.rs::a_key",
        "the two words a hook reports a key in, read as the position and the transition they hold",
    ),
    (
        "host-windows/capture.rs::room_for",
        "a count of bytes read as the buffer a raw input is read into",
    ),
    ("host-macos/capture.rs::the_device", A_HANDLE_EITHER_WAY),
    ("host-macos/capture.rs::the_queue", A_HANDLE_EITHER_WAY),
    ("host-macos/capture.rs::the_array", A_HANDLE_EITHER_WAY),
    ("host-macos/capture.rs::the_capture", A_HANDLE_EITHER_WAY),
    (
        "host-macos/capture.rs::Capture::next_ask",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::answer_the_ask",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::keep_the_arrivals",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::keep",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::forget",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::keep_what_was_seized",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::keep_the_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::a_queue_has_values",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::a_device_has_gone",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::next_thing_with_values",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::done_with",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::next_gone",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::its_queue",
        THE_CALLBACKS_OWN_LISTS,
    ),
    (
        "host-macos/capture.rs::Capture::forget_it",
        THE_CALLBACKS_OWN_LISTS,
    ),
    ("host-macos/capture.rs::on_value", THE_CALLBACKS_OWN_LISTS),
    ("host-macos/capture.rs::on_removed", THE_CALLBACKS_OWN_LISTS),
    ("host-macos/capture.rs::on_matched", THE_CALLBACKS_OWN_LISTS),
    (
    ...
    (
        "host-macos/supervisor.rs::a_descriptor",
        "the descriptor a variable holds, and `-1` where it holds nothing a descriptor could be",
    ),
    (
        "host-windows/supervisor.rs::a_handle",
        "the handle a variable holds, and zero where it holds nothing a handle could be",
    ),
    (
        "host-macos/supervisor.rs::Supervisor::is_supervised",
        "the two descriptors read as whether somebody handed either over, which is what `-1` \
         says on this machine",
    ),
    (
        "host-windows/supervisor.rs::Supervisor::is_supervised",
        "the two handles read as whether somebody handed either over, which is what zero says \
         here",
    ),
    (
        "host-windows/supervisor.rs::Supervisor::watches_probes",
        "the probe handle read as whether there is one, likewise",
    ),
    (
        "host-macos/control.rs::a_path",
        "the path a variable holds, and nothing where it holds nothing",
    ),
    (
        "host-macos/region.rs::a_descriptor",
        "the descriptor a variable holds, and nothing where it holds anything else",
    ),
    (
        "host-windows/region.rs::a_handle",
        "the handle a variable holds, likewise",
    ),
    (
        "host-macos/region.rs::Region::bytes",
        "the pages a mapping landed in, as the slice `engine` writes a trace into",
    ),
    (
        "host-windows/region.rs::Region::bytes",
        "the pages a mapping landed in, likewise",
    ),
    (
        "host-macos/region.rs::Region::snapshot",
        "a copy of those pages, taken rather than borrowed because the process writing them is \
         still running",
    ),
    (
        "host-macos/cf.rs::Owned::drop",
        "the reference given back, which is the only thing that type is for",
    ),
    (
        "host-macos/cf.rs::Owned::pointer",
        "a value there is one of, read as the pointer the calls take",
    ),
    (
        "host-windows/mdns.rs::went",
        "a call's own answer read as whether it did what it was asked",
    ),
    (
        "host-windows/mdns.rs::sent",
        "the same reading of a call that answers with a count instead",
    ),
    (
        "host-macos/vhid.rs::Connection::clone",
        "the two handles on the connection shared, which is what the loop reading it and the \
         writes each hold one of",
    ),
    (
        "host-macos/vhid.rs::keyboard_parameters",
        "the virtual keyboard's own identity as the payload the request carries it in, which is \
         this crate's to agree with itself about since capture leaves that device alone by it",
    ),
    (
        "host-macos/inject.rs::Nowhere::send_report",
        "a report read as the number a write answers with, for a run with no device to write it \
         to: nothing is asked of any machine, so `0` is the whole of what there is to say",
    ),
    (
        "host-macos/cf.rs::CfString::new",
        "text as one of Core Foundation's own strings, which is what a property name reaches it \
         as",
    ),
    (
        "host-macos/cf.rs::device_number",
        "a property of a device already in hand, read as the integer it holds: the name goes in \
         as a Core Foundation string and the value comes out by asking it what it is",
    ),
    (
        "host-macos/cf.rs::device_string",
        "the same property read as the text it holds",
    ),
    (
        "host-macos/repeat.rs::Parameters::drop",
        "the reference given back, likewise",
    ),
    (
        "host-macos/acceleration.rs::CfNumber::fixed",
        "a number as the 16.16 fixed-point value these properties take, which is a multiply and \
         the Core Foundation value around it",
    ),
    (
    ...
    (
        "host-windows/capture.rs::the_front",
        "the front of what the turn put down, read as whether it came from a device and the \
         number the run gave that device",
    ),
    (
        "host-windows/capture.rs::taken",
        "the front of what the turn put down, taken, as the event the run reads",
    ),
    (
    ...
    (
        "host-windows/mdns.rs::Network::a_sink_at",
        "four bytes and a port read as the machine they name, likewise",
    ),
    (
        "host-windows/lib.rs::WindowsHost::a_sink_at",
        "four bytes and a port read as the machine they name, with nothing asked of a resolver",
    ),
    (
        "host-windows/mdns.rs::origin",
        "an origin a run may not have started yet, read as the moment a reading is against",
    ),
    (
        "host-windows/mdns.rs::at",
        "four bytes and a port read as the machine they name, with nothing asked of a resolver",
    ),
    (
    ...
    (
        "host-windows/mdns.rs::the_first_of",
        "what a resolver answered, read as the one address a run uses out of however many it \
         named",
    ),
    (
    ...
    (
        "host-macos/capture.rs::came_off",
        "the iterator a listing came off as the thing the next finding is asked of, and the \
         zero of none where the machine gave no listing at all",
    ),
    (
...
/// The same, over what a release build holds.
///
/// A stand-in a test builds is not the thing a rule is about: what it plays is
/// whatever the case needed, and holding it to the shape of the platform's own
/// half would be a rule about the assertions rather than about the run.
///
/// Inside function bodies as well as inside modules, because a type declared in
/// one is as much a part of the build as a type beside it — and a rule that read
/// only the items a module holds would be one a body could be moved into.
fn walk_items_built(items: &[syn::Item], each: &mut impl FnMut(&syn::Item)) {
    for item in items {
        each(item);
        match item {
            syn::Item::Mod(module) => {
                let tested = module
                    ...
                    .iter()
                    .any(|attribute| attribute.path().is_ident("cfg") && cfg_is_test(attribute));
                if let (false, Some((_, inner))) = (tested, &module.content) {
                    walk_items_built(inner, each);
                }
            }
            syn::Item::Fn(function) => walk_items_built(&items_in(&function.block), each),
            syn::Item::Impl(block) => {
                for member in &block.items {
                    if let syn::ImplItem::Fn(function) = member {
                        walk_items_built(&items_in(&function.block), each);
                    }
                }
            }
            _ => {}
...
/// The items one body declares, in order.
fn items_in(block: &syn::Block) -> Vec<syn::Item> {
    block
        .stmts
        .iter()
