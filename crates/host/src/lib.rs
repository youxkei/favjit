...
pub trait Host {
    ...
    /// Put one line where the person who started this run reads it.
    ///
    /// Apart from [`Host::warn`], because the two are read in different places
    /// and one of them is not a problem: what a run that changes nothing outside
    /// itself has to show is its whole output, and a run whose output went to the
    /// log beside the warnings would be a mode whose answer is buried in it.
    ///
    /// The words and which of them are worth saying twice are the run's, so what
    /// a host does with this is put it somewhere and nothing else (ADR-0006).
    fn say(&mut self, line: core::fmt::Arguments);

    /// Stop this thread and do not come back.
    ///
    /// The machine is what ends a thread parked this way, so a host that answers
    /// is one nothing is holding — which a run reads as the parking having done
    /// nothing rather than as a thread to keep asking about.
    fn park(&mut self);
...
pub trait DiscoveryHost {
    ...
    /// The machine a literal address and a port name, with no resolver asked.
    ///
    /// Its own operation rather than a case inside the one below, because which
    /// of the two an answer wants is a decision: a responder that sent the
    /// address along has saved a lookup, and whether to spend one is the run's
    /// to settle rather than something a machine works out from an argument
    /// being absent (ADR-0006).
    fn a_sink_at(&mut self, address: [u8; 4], port: u16) -> Option<Sink>;

    /// The machine a name and a port resolve to, which is a resolver asked.
    ///
    /// Not on [`Searching`]: it reaches a resolver and not that socket, and it is
    /// asked once a lookup has already answered.
    fn resolve_discovered(&mut self, host: &str, port: u16) -> Option<Sink>;
}
...
pub mod sink {
    ...
    pub trait SinkInputHost: Host {
        ...
        /// `engine`'s.
        ///
        /// Take the clock the captured stream's own timestamps will be on.
        ///
        /// Asked for before the loop below rather than inside it, because
        /// reading the clock and starting that loop are two calls into the
        /// machine and a host makes one of those per operation: which of them
        /// happens first is the order a run drives (ADR-0006), and it matters —
        /// a clock taken after the loop began would be behind the first
        /// keystroke stamped against it.
...
pub mod output {
    ...
    pub trait OutputHost: Send {
        ...
        /// Put this whole frame on the connection, answering `0` for the write
        /// that went and this machine's own error number otherwise.
        ///
        /// The number and not whether it went, for
        /// [`crate::sink::SinkHost::send_report`]'s reason: a connection the
        /// service has closed and a write that ran out of time are the same
        ...
        /// connection is read from afterwards holds this number (ADR-0009).
        ///
        /// The bytes and not which frame they are, because the framing is one
        /// fact and the run already holds it: a length is counted off the front
        /// of everything arriving here, so a machine counting one onto the front
        /// of everything leaving would be the same rule written twice — where
        /// the two can come to disagree, and what says so is a service that
        /// stops answering (ADR-0006).
...
pub mod source {
    ...
    pub trait Answers {
        ...
        /// Show this machine these keys, in this order, as though somebody had
        /// pressed them.
        ///
        /// The whole sequence in one call and not a call per stroke, because
        /// what a machine makes of a key depends on what came before it: the
        /// order is the behaviour, so which strokes there are and in what order
        /// is the run's, and which keys they are is the platform's ([`Shown`],
        /// ADR-0006).
        ///
        /// What the platform owes beyond sending them is that its own procedures
        /// can tell these from a key somebody pressed — otherwise a run reads
        /// its own strokes back and relays them.
    ...
    /// A key the run shows this machine, by what it is for rather than which key
    /// it is.
    ///
    /// Two of them, because that is the whole of what the run has to say: one is
    /// the modifier a chord is made with, which the machine is also asked about
    /// ([`Answers::the_modifier_is_down`]), and the other is whatever that
    /// machine acts on in no way. Which keys those are is the platform's, the
    /// same way a usage page is — a run naming a virtual key would be deciding
    /// for a machine it cannot see (ADR-0006).
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    #[non_exhaustive]
    pub enum Shown {
        /// The modifier a chord is made with.
        TheModifier,
        /// A key that machine acts on in no way, whose whole purpose is to be
        /// something rather than nothing.
        AKeyNothingActsOn,
    }

    /// One of them going down, or coming back up.
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    pub struct Stroke {
        pub key: Shown,
        pub released: bool,
    ...
    pub trait SourceHost: Host + Entropy + DiscoveryHost + crate::IdentityStore {
        ...
        /// Take the clock the captured stream's own timestamps will be on.
        ///
        /// Asked for before the reading below rather than inside it, because
        /// reading the clock and starting the thread are two calls into the
        /// machine and a host makes one of those per operation: which of them
        /// happens first is the order a run drives (ADR-0006), and it matters —
        /// a clock taken after that thread began would be behind the first
        /// keystroke stamped against it.
        fn start_a_clock(&mut self);
        ...
        /// Show this machine a press of the modifier the chord is made with, so
        /// that it holds what the finger on it is holding.
        ///
        /// The mirror of what the sink is told as the keyboard goes over: the
        /// press that made the chord was refused, so this machine never saw it,
        /// and a machine that believes the modifier is up answers
        /// [`Answers::the_modifier_is_down`] with `false` for as long as the
        /// finger stays — which is the chord's own letter no longer being
        /// recognised and the next key arriving bare.
        ///
        /// Show this machine these keys, in this order, the way
        /// [`Answers::show_here`] does — asked of the loop rather than of a
        /// procedure, because what the loop shows it is decided where the
        /// keyboard moves and not inside a call the machine made.
...
pub mod capture {
    ...
    /// The front of what a turn put down, as the two facts that say whether the
    /// run can name it yet.
    ///
    /// Both at once rather than a kind saying which case it is, so that what
    /// reads them has nothing to ask about which it was: a report names a device
    /// and carries a number, or it names a device and carries none, or it names
    /// no device at all.
    #[derive(Debug, Clone, Copy, PartialEq)]
    pub struct Waiting {
        /// Whether it came from a device at all, as against being something the
        /// machine said that names none.
        pub from_a_device: bool,
        /// The number the run gave the device it came from, and none while the
        /// run has given it none.
        pub numbered: Option<DeviceId>,
    }
    ...
    pub trait SourceCapture: Reading {
        ...
        /// The front of what the turn just taken put down, read as whether the
        /// run can name it — and none once nothing is waiting.
        ///
        ...
        ///
        /// Read without taking it, so that what to do about a report whose
        /// device carries no number is the run's: a call that took it would have
        /// decided, and the only two things left to do with it there are to drop
        /// it and to put it back.
        fn what_is_waiting(&mut self) -> Option<Waiting>;

        /// Take the front of what the turn put down, as the event the run reads.
    ...
    pub trait SinkCapture: Reading {
        ...
        /// The next registry entry the listing above has, as the thing the
        /// calls below are made on.
        ///
        ...
        /// (ADR-0006).
        fn next_device_here(&mut self) -> Option<crate::Handed>;

        /// The next registry entry that has arrived since, off what
        /// [`SinkCapture::ask_to_be_told_about_devices`] asked for.
        ///
        /// Its own call rather than one that answers off whichever listing still
        /// has something, because which of the two a turn reads is the whole of
        /// whether a keyboard plugged in later is ever converted, and that is
        /// `engine`'s to decide (ADR-0006). A device that arrived between the
        /// asking and the listing is on both, and the run tells the second
        /// finding of it by the name it goes under.
        fn next_arriving_device(&mut self) -> Option<crate::Handed>;
