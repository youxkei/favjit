//! How the forwarding machine finds the machine it relays to (ADR-0012).
//!
...
//!
//! The format is here for the same reason the link's is: it is what the two
...
//!
//! A crate of its own rather than a module of `engine`, because the simulator writes
//! answers as well as `engine` reads them. Both need the same wire vocabulary without
//! making a host depend on `engine` or making `engine` depend on its test double.
...
/// (`favjit_engine::link::SERVICE`, `favjit_engine::link::PAIRING`).
...
pub use favjit_host::Discovered as Found;

/// A query for every instance of `name`.
pub fn question(name: &str) -> Vec<u8> {
...
pub fn answer(message: &[u8], name: &str) -> Option<Found> {
    let records = read(message)?;
    let service = service(name).trim_end_matches('.').to_ascii_lowercase();
